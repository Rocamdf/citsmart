...
public class ComplementoSLA_Update {
	public String execute(HashMap mapFields, TransactionControler transactionControler) throws ServiceException, Exception{
		...
		if (idServicoContrato != null){
        		...
        		Collection col = prioridadeServicoUnidadeDao.findByIdServicoContrato(idServicoContrato);
        		if (col != null){ //Coloca fim para todos os itens associados.
        		    for (Iterator it = col.iterator(); it.hasNext();){
	        			...
	        			if (prioridadeServicoUnidadeDTO.getDataFim() == null){
	        			    prioridadeServicoUnidadeDTO.setDataFim(UtilDatas.getDataAtual());
        		...
        		if (colUnidades != null){
        			...
        			if (prioridade != null && !prioridade.trim().equalsIgnoreCase("")){
        			    ...
        			    prioridadeServicoUnidadeDTO.setDataFim(null);
        		...
        		Collection col = prioridadeAcordoNivelServicoDao.findByIdAcordoNivelServico(idAcordoNivelServico);
        		if (col != null){ //Coloca fim para todos os itens associados.
        		    for (Iterator it = col.iterator(); it.hasNext();){
	        			PrioridadeAcordoNivelServicoDTO prioridadeAcordoNivelServicoDTO = (PrioridadeAcordoNivelServicoDTO)it.next();
		...
		if(tipoAcordo != null && tipoAcordo.equalsIgnoreCase("F")){
			...
			String removed = (String) mapFields.get("REMOVED");
			if(removed == null){
				removed = "";
			}
			if(valorSerializado!=null){
				...
				boolean flag = removeValorAjusteCadastrados(valorAjusteDAO, idAcordoNivelServico);
				if(flag && (removed.equalsIgnoreCase("false") || removed.equalsIgnoreCase(""))){
		...
		if(tipoAcordo != null && tipoAcordo.equalsIgnoreCase("R")){
			String valorSerializado = (String) mapFields.get("RESULTADOSESPERADOSSERIALIZADO");
			String removed = (String) mapFields.get("REMOVED");
			if(removed == null){
				removed = "";
			}
			if(valorSerializado!=null){
				List<ResultadosEsperadosDTO> listResp = (List) WebUtil.deserializeCollectionFromString(ResultadosEsperadosDTO.class, valorSerializado);
				...
				boolean flag = removeResultadoCadastrados(resultadosEsperadosDAO, idAcordoNivelServico);
				if(flag && (removed.equalsIgnoreCase("false") || removed.equalsIgnoreCase(""))){
		...
		String removed = (String) mapFields.get("REMOVED");
		if(removed != null && removed.equalsIgnoreCase("true")){
			acordoNivelServicoAux.setDeleted("y");
		}
	...
	private boolean removeValorAjusteCadastrados(ValorAjusteGlosaDAO valorAjusteDAO, Integer idAcordoNivelServico) {
		boolean flag = false;
		try {
			valorAjusteDAO.deleteByIdAcordoNivelServico(idAcordoNivelServico);
			flag = true;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private boolean removeResultadoCadastrados(ResultadosEsperadosDAO resultadosEsperadosDAO, Integer idAcordoNivelServico) {
		boolean flag = false;
		try {
			resultadosEsperadosDAO.deleteByIdAcordoNivelServico(idAcordoNivelServico);
			flag = true;
		} catch (Exception e) {
			e.printStackTrace();
		}
