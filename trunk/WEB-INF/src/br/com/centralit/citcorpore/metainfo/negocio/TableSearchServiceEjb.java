...
public class TableSearchServiceEjb extends CrudServicePojoImpl implements TableSearchService {

	private static final long serialVersionUID = -2686086440799727966L;
	...
	public String findItens(TableSearchDTO parm, boolean tableVinc, HashMap map) throws Exception {
		...
		if (strSGBDPrincipal.equalsIgnoreCase("ORACLE") || strSGBDPrincipal.equalsIgnoreCase("ORACLE")) {
			...
			try {
				lookupDao.execSQL(sql1, null);
			} catch (Exception e) {
				// e.printStackTrace();
			}
			String sql2 = "alter session set nls_sort = binary_ai";
			try {
				lookupDao.execSQL(sql2, null);
			} catch (Exception e) {
				// e.printStackTrace();
			}
		}

		Collection colRetorno = lookupDao.execSQL(sql, null);
							...
							if (colDominios != null) {
								...
								for (Iterator itDominios = colDominios.iterator(); itDominios.hasNext();) {
									...
									if (objs[i].toString().equalsIgnoreCase(valorVisaoCamposNegocioDTO.getValor())) {
										// valor = objs[i].toString() + " [" + valorVisaoCamposNegocioDTO.getDescricao() + "] ";
										valor = objs[i].toString();
										break;
									}
								}
							...
							if (grupoVisaoDTO.getColCamposVisao() != null) {
								for (Iterator it3 = grupoVisaoDTO.getColCamposVisao().iterator(); it3.hasNext();) {
									...
									if (valorApres.trim().equalsIgnoreCase("")) {
										...
										valorApres = MetaUtil.recuperaValorAtualCampo(colCamposPKPrincipal, grupoVisaoCamposNegocioDTO.getCamposObjetoNegocioDto(), tablePrincipal, obj,
												matrizVisaoDTO, map);
									}
									if (!prim) {
										retorno = retorno + ",";
	...
	private String generateSQLVinculos(Integer idVisaoRelacionada, HashMap map) throws Exception {
		if (map == null) {
			return "";
		}
		CamposObjetoNegocioDao camposObjetoNegocioDao = new CamposObjetoNegocioDao();
