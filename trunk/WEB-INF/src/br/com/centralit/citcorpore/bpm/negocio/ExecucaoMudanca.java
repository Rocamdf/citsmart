...
//	}
	...
	public void calculaTempoCaptura(RequisicaoMudancaDTO requisicaoMudancaDto) throws Exception {
		...
		calculoDto = new CalendarioServiceEjb().calculaPrazoDecorrido(calculoDto, requisicaoMudancaDto.getDataHoraCaptura());
	...
	public void enviaEmailGrupo(Integer idModeloEmail, Integer idGrupoDestino) throws Exception {
		...
		try {
			...
			Collection<GrupoEmailDTO> grupoDeEmails = (Collection<GrupoEmailDTO>) grupoEmailService.findByIdGrupo(idGrupoDestino);
			for(GrupoEmailDTO contatoEmail : grupoDeEmails) {
				String nomeContato = contatoEmail.getNome();
				String emailContato = contatoEmail.getEmail();
				requisicaoMudancaDTO.setNomeContato(nomeContato);
				mensagem = new MensagemEmail(idModeloEmail, new IDto[] { requisicaoMudancaDTO });
				mensagem.envia(emailContato, remetente, remetente);
			}
