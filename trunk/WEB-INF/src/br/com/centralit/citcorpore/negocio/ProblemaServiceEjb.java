...
public class ProblemaServiceEjb extends CrudServicePojoImpl implements ProblemaService {
	...
	private SolicitacaoServicoProblemaDao solicitacaoServicoProblemaDao;
	...
	private SolicitacaoServicoProblemaDao getSolicitacaoServicoProblemaDao() {
		if (solicitacaoServicoProblemaDao == null) {
			solicitacaoServicoProblemaDao = new SolicitacaoServicoProblemaDao();
		}
		return solicitacaoServicoProblemaDao;
	}

	private ProblemaDAO getProblemaDao() {
		if (problemaDao == null) {
	...
	public IDto create(IDto model) throws ServiceException, LogicException {
		...
		try {
			...
			if(problema.getIdGrupo() == null){
				if(categoriaProblemaDto!=null){
					if(categoriaProblemaDto.getIdGrupoExecutor()!=null){
						problema.setIdGrupoNivel1(categoriaProblemaDto.getIdGrupoExecutor());
			...
			if(!problema.getTitulo().equalsIgnoreCase("Problema Criado por Rotina automática")){
				...
				if (resultado == false) {
					throw new LogicException(i18n_Message("Problema.grupoSemPermissao"));
				}
			} else{
				...
				try {
						...
						if(problema.getStatus()!=null && problema.getStatus().equalsIgnoreCase(SituacaoRequisicaoProblema.EmInvestigacao.getDescricao())){
							if(problema.getCausaRaiz()!=null){
								if(problema.getCausaRaiz().equalsIgnoreCase("")){
									throw new LogicException(i18n_Message("problema.campoObrigatorioCausaRaiz"));
								}
							}
							if(problema.getSolucaoContorno()!=null){
								if(problema.getSolucaoContorno().equalsIgnoreCase("")){
									throw new LogicException(i18n_Message("problema.campoObrigatorioSolucaoContorno"));
								}
							}
					...
					if (problema != null) {
						if (problema.getListIdSolicitacaoServico() != null && problema.getListIdSolicitacaoServico().size() > 0) {
							...
							SolicitacaoServicoProblemaDTO solicitacaoServicoProblemaDTO = new SolicitacaoServicoProblemaDTO();
							SolicitacaoServicoProblemaService solicitacaoServicoProblemaService = (SolicitacaoServicoProblemaService) ServiceLocator.getInstance().getService(SolicitacaoServicoProblemaService.class, null);
							solicitacaoServicoProblemaDTO = solicitacaoServicoProblemaService.restoreByIdProblema(problema.getIdProblema());
							if (solicitacaoServicoProblemaDTO != null && solicitacaoServicoProblemaDTO.getIdProblema() != null && solicitacaoServicoProblemaDTO.getIdProblema().SIZE > 0) {
						...
						if(problema.getListProblemaItemConfiguracaoDTO()!=null && problema.getListProblemaItemConfiguracaoDTO().size() > 0){
							...
							if (problemaItemConfiguracaoRestoreDTO != null && problemaItemConfiguracaoRestoreDTO.getIdProblema() != null && problemaItemConfiguracaoRestoreDTO.getIdProblema().SIZE > 0) {
						...
						if(problema.getListProblemaMudancaDTO()!=null){
							...
							if (problemaMudancaRestoreDTO != null && problemaMudancaRestoreDTO.getIdProblema() != null && problemaMudancaRestoreDTO.getIdProblema().SIZE > 0) {
					...
					if(problema.getAcaoFluxo().equalsIgnoreCase("E")){						
						if(problema.getStatus()!=null && (problema.getStatus().equalsIgnoreCase(SituacaoRequisicaoProblema.Resolucao.getDescricao()) || problema.getStatus().equalsIgnoreCase(SituacaoRequisicaoProblema.Concluida.getDescricao()))){
	...
	public void createAll(ProblemaDTO problema, ArrayList<ProblemaItemConfiguracaoDTO> ics, ArrayList<ProblemaMudancaDTO> rdm, UsuarioDTO usuarioDto) throws ServiceException, LogicException {
		...
		SolicitacaoServicoProblemaDao solicitacaoServicoProblemaDao = new SolicitacaoServicoProblemaDao();
		TransactionControler tc = new TransactionControlerImpl(getProblemaDao().getAliasDB());
		...
		//Parte comentanda na espera de como determinados campos vao ser realmente gravados
		//da forma com estava implementado, serviço estava relacionado a problema
		//Muito do esta aqui talvez deverá ser implementado de outra maneira
		//analisar se ainda sera relacionado o serviço e como essas informaçoes impactarão o fluxo
/*		ProblemaDTO problemaDto = new ProblemaDTO();
		problemaDto.setIdSolicitante(problema.getIdSolicitacaoServico());
		problemaDto.setUsuarioDto(usuarioDto);
		problemaDto.setIdContrato(problema.getIdContrato());
		problemaDto.setIdServico(problema.getIdServico());
		...
		try {
			...
			problemaDto.setIdServicoContrato(servicoContratoDTO.getIdServicoContrato());
			...
			problemaDto.setUrgencia(problema.getUrgencia().substring(0,1));
			problemaDto.setImpacto(problema.getImpacto().substring(0,1));
			
			//Foi comentado pois a prioridade/prazo não será determinado por aqui -> David
//			problemaServiceEjb.determinaPrioridadeEPrazo(problemaDto, tc);
			
			problema.setIdPrioridade(problemaDto.getIdPrioridade());
			problema.setPrazoHH(problemaDto.getPrazoHH());
			problema.setPrazoMM(problemaDto.getPrazoMM());
			problema.setSlaACombinar(problemaDto.getSlaACombinar());
			problema.setDataHoraLimite(problemaDto.getDataHoraLimite());
		} catch (Exception e1) {
			e1.printStackTrace();
		}
*/		
		...
		try {
			this.getProblemaDao().setTransactionControler(tc);
			...
			problema = (ProblemaDTO) this.getProblemaDao().create(problema);
			...
			if (problema.getAdicionarBDCE() != null && problema.getAdicionarBDCE().trim().equalsIgnoreCase("S")) {
				...
				getProblemaDao().update(problema);
			}
	...
	public ProblemaDTO restoreAll(Integer idProblema, TransactionControler tc) throws Exception {
		...
		problemaDto = (ProblemaDTO) problemaDao.restore(problemaDto);
		
		try {
			
		} catch (Exception e) {
			throw new Exception(i18n_Message("solicitacaoservico.erro.recuperardadosolicitacao") + " " + idProblema);
		}
	...
	public void updateAll(ProblemaDTO problema, ArrayList<ProblemaItemConfiguracaoDTO> ics, ArrayList<ProblemaMudancaDTO> rdm, UsuarioDTO usuarioDto) throws ServiceException, LogicException {
		...
		try {
			ProblemaDAO problemaDAO = new ProblemaDAO();
			ProblemaDTO problemaAux = (ProblemaDTO) problemaDAO.restore(problema);
			if (problemaAux == null){
		...
		try {
			getProblemaDao().setTransactionControler(tc);
			...
			/*if (rdm != null) {
				...
				for (ProblemaMudancaDTO problemaMudanca : rdm) {	
					ArrayList<ProblemaMudancaDTO> listaProblema =  (ArrayList<ProblemaMudancaDTO>) getProblemaMudancaDao().findByIdProblema(problema.getIdProblema());	
					for(ProblemaMudancaDTO aux : listaProblema){
						if (aux == null) {
							problemaMudanca.setIdProblema(problema.getIdProblema());
							getProblemaMudancaDao().create(problemaMudanca);
						} else {
							problemaMudanca.setIdProblemaMudanca(aux.getIdProblemaMudanca());
							problemaMudanca.setIdProblema(problema.getIdProblema());
							getProblemaMudancaDao().update(problemaMudanca);
						}
					}
...
//			getProblemaMudancaDao().deleteByIdProblema(problema.getIdProblema());
			...
			if (problema != null && problema.getAdicionarBDCE() != null && problema.getAdicionarBDCE().trim().equalsIgnoreCase("S")) {
				if (problema.getIdBaseConhecimento() != null) {
					...
					getProblemaDao().updateNotNull(problema);
				} else {
	...
	public List<ProblemaDTO> listProblema(ProblemaDTO bean) throws ServiceException, LogicException {
		...
		try {
			return (List<ProblemaDTO>) getProblemaDao().listProblema(bean);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void deleteAll(ProblemaDTO problema) throws ServiceException, LogicException {
		...
		try {
			...
			getProblemaDao().delecaoLogica(problema);
			// getProblemaItemConfiguracaoDao().delecaoLogicaByIdProblema(problema);
	...
	public Collection findByPrioridade(Integer parm) throws Exception {
		ProblemaDAO dao = new ProblemaDAO();
		try {
			return dao.findByPrioridade(parm);
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
	...
	public Collection findByIdCriador(Integer parm) throws Exception {
		ProblemaDAO dao = new ProblemaDAO();
		try {
			return dao.findByIdCriador(parm);
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
	...
	public Collection findByIdProprietario(Integer parm) throws Exception {
		ProblemaDAO dao = new ProblemaDAO();
		try {
			return dao.findByIdProprietario(parm);
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
	...
	public void deleteByIdProprietario(Integer parm) throws Exception {
		ProblemaDAO dao = new ProblemaDAO();
		try {
			dao.deleteByIdProprietario(parm);
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
	...
	public Collection findByDescricao(String parm) throws Exception {
		ProblemaDAO dao = new ProblemaDAO();
		try {
			return dao.findByDescricao(parm);
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
	...
	public Collection findByUrgencia(String parm) throws Exception {
		ProblemaDAO dao = new ProblemaDAO();
		try {
			return dao.findByUrgencia(parm);
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
	...
	public Collection findByProativoReativo(String parm) throws Exception {
		ProblemaDAO dao = new ProblemaDAO();
		try {
			return dao.findByProativoReativo(parm);
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
	...
	public void deleteByProativoReativo(String parm) throws Exception {
		ProblemaDAO dao = new ProblemaDAO();
		try {
			dao.deleteByProativoReativo(parm);
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
	...
	public Collection findByMsgErroAssociada(String parm) throws Exception {
		ProblemaDAO dao = new ProblemaDAO();
		try {
			return dao.findByMsgErroAssociada(parm);
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
	...
	public void deleteByMsgErroAssociada(String parm) throws Exception {
		ProblemaDAO dao = new ProblemaDAO();
		try {
			dao.deleteByMsgErroAssociada(parm);
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
	...
	public Collection findByIdErrosConhecidos(Integer parm) throws Exception {
		ProblemaDAO dao = new ProblemaDAO();
		try {
			return dao.findByIdErrosConhecidos(parm);
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
	...
	public void deleteByIdErrosConhecidos(Integer parm) throws Exception {
		ProblemaDAO dao = new ProblemaDAO();
		try {
			dao.deleteByIdErrosConhecidos(parm);
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
	...
	public Collection findByIdProblemaIncidente(Integer parm) throws Exception {
		ProblemaDAO dao = new ProblemaDAO();
		try {
			return dao.findByIdProblemaIncidente(parm);
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
	...
	public void deleteByIdProblemaIncidente(Integer parm) throws Exception {
		ProblemaDAO dao = new ProblemaDAO();
		try {
			dao.deleteByIdProblemaIncidente(parm);
		} catch (Exception e) {
			throw new ServiceException(e);
		}
	}
		...
		try {
			...
			if(colAux != null){
				for(SolicitacaoServicoProblemaDTO sol : colAux){
					...
					colProblema.add((ProblemaDTO) problemaDao.restore(problema));
				}
			}
	...
	public Collection findBySolictacaoServico(ProblemaDTO bean) throws ServiceException, LogicException {
		...
		try {
			return getProblemaDao().listProblemaByIdSolicitacao(bean);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public List<ProblemaDTO> listProblemaByIdItemConfiguracao(Integer idItemConfiguracao) throws PersistenceException {
		...
		ProblemaDAO problemaDao = new ProblemaDAO();
		try {
			return problemaDao.listProblemaByIdItemConfiguracao(idItemConfiguracao);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public Collection<ProblemaDTO> quantidadeProblemaPorBaseConhecimento(ProblemaDTO problema) throws Exception {
		ProblemaDAO problemaDAO = new ProblemaDAO();
		return problemaDAO.quantidadeProblemaPorBaseConhecimento(problema);
	}
	...
	public Collection<ProblemaDTO> listaProblemaPorBaseConhecimento(ProblemaDTO problema) throws Exception {
		...
		ProblemaDAO problemaDao = new ProblemaDAO();
		try {
			listaProblemaPorBaseConhecimento = problemaDao.listaProblemasPorBaseConhecimento(problema);
	...
	public Collection findByConhecimento(BaseConhecimentoDTO baseConhecimentoDto) throws ServiceException, LogicException {
		...
		try {
			return problemaDao.findByConhecimento(baseConhecimentoDto);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	//SE FOR UTILIZAR IDPROBLEMAPAI, ENTAO DESCOMENTAR ESSE METODO -> DAVID
//	@Override
//	public Collection<ProblemaDTO> listProblemasFilhos() throws Exception {
//		ProblemaDAO problemaDao = new ProblemaDAO();
//
//		return problemaDao.listProblemasFilhos();
//	}
	...
	public Collection<ProblemaDTO> listByTarefas(Collection<TarefaFluxoDTO> listTarefas) throws Exception {
		...
		listProblemaDto = problemaDao.listByTarefas(listTarefas);
	...
	public ProblemaDTO restoreByIdInstanciaFluxo(Integer idInstanciaFluxo) throws Exception {
		...
		try {
			problemaDTO = (ProblemaDTO) problemaDao.restoreByIdInstanciaFluxo(idInstanciaFluxo);
		} catch (Exception e) {
	...
	public ContatoProblemaDTO criarContatoProblema(ProblemaDTO problemaDto, TransactionControler tc) throws ServiceException, LogicException {
		...
		ProblemaDAO proplemaDao = new ProblemaDAO();
	...
	problemaDto = (ProblemaDTO) getProblemaDao().restore(problemaDto);
	String res = problemaDto.getStatus();
	return res;
	
	}
	
	private void determinaPrazo(ProblemaDTO problemaoDto, Integer idCalendarioParm) throws Exception {
		if (problemaoDto.getDataHoraLimiteSolucionar() == null)
			throw new LogicException(i18n_Message("citcorpore.comum.Data/horaTerminoNaoDefinida"));

		CalculoJornadaDTO calculoDto = new CalculoJornadaDTO(idCalendarioParm, problemaoDto.getDataHoraCaptura());
		//Para evitar null pointer, foi adicionado a condição abaixo -> david.junior
		if(calculoDto.getDataHoraInicial() != null){
			calculoDto = new CalendarioServiceEjb().calculaPrazoDecorrido(calculoDto, problemaoDto.getDataHoraLimite());
			problemaoDto.setPrazoHH(calculoDto.getTempoDecorridoHH());
			problemaoDto.setPrazoMM(calculoDto.getTempoDecorridoMM());
		}
	}
	...
	/**
	 * Método responsável por validar a dinâmica do fluxo com relação ao registro de erros conhecidos.
	 * Esse método deve garantir que o usuário só avance a partir da fase de avaliação e diagnóstico para a fase de resolução
	 * após ter registrado pelo menos um erro (fase de registro de erros conhecidos).
	 * 
	 * @autor thiago.monteiro 
	 * @param ProblemaDTO problemaDTO
	 * @param TransactionControler tc
	 * @throws Exception
	 * @return
	 */
...
//						 Alterando o valor da condição para o id da base conhecimento.
	...
	/**
	 * Método responsável por notificar que o prazo de contorno/solução de um problema expirou.
	 * O prazo de contorno/solução irá expirar quando a data hora atual for posterior a data hora limite do problema.
	 * A notificação deve ser feita para o proprietário (quem capturou o problema) e o grupo de interessados (grupo executor).
	 * 
	 * @autor thiago.monteiro
	 * @param ProblemaDTO problemaDTO
	 * @throws Exception
	...
	public int contarProblemasComPrazoLimiteContornoExpirado(UsuarioDTO usuarioDTO) throws Exception {
		 return ( (ProblemaDAO) getDao() ).contarProblemasComPrazoLimiteContornoExpirado(usuarioDTO);
	}
...
//						
	...
	public Collection<ProblemaDTO> listaProblemasComPrazoLimiteContornoExpirado(UsuarioDTO usuarioDTO) throws Exception {
		ProblemaDAO problemaDao = new ProblemaDAO();
		return problemaDao.listaProblemasComPrazoLimiteContornoExpirado(usuarioDTO);
	}
	...
	public Collection<PesquisaProblemaDTO> listProblemaByCriterios(PesquisaProblemaDTO pesquisaProblemaDto) throws Exception {
		...
		Collection<PesquisaProblemaDTO> listProblemaByCriterios = getProblemaDao().listProblemaByCriterios(pesquisaProblemaDto);
	...
	public Collection<RelatorioProblemaIncidentesDTO> listProblemasIncidentes(RelatorioProblemaIncidentesDTO relatorioProblemaIcidentesDto) throws Exception {
		...
		Collection<RelatorioProblemaIncidentesDTO> listAux = getProblemaDao().listProblemaIncidentes(relatorioProblemaIcidentesDto);
	...
	public ProblemaDTO restoreAll(ProblemaDTO problema, TransactionControler tc) throws Exception {
		...
		problemaDto = (ProblemaDTO) problemaDao.restoreTela(problema);
		
		try {
			
		} catch (Exception e) {
			throw new Exception(i18n_Message("solicitacaoservico.erro.recuperardadosolicitacao") + " " + problema.getIdProblema());
		}
	...
	public ProblemaDTO restauraTodos(ProblemaDTO param) throws Exception {
		...
		ProblemaDAO problemaDAO = new ProblemaDAO();
		return (ProblemaDTO) problemaDAO.restauraTodos(param);
	...
	public Collection<RelatorioQuantitativoProblemaDTO> listaQuantidadeProblemaPorSituacao(RelatorioQuantitativoProblemaDTO relatorioQuantitativoProblemaDto) throws Exception {
		return getProblemaDao().listaQuantidadeProblemaPorSituacao(relatorioQuantitativoProblemaDto);
	}
	...
	public Collection<RelatorioQuantitativoProblemaDTO> listaQuantidadeProblemaPorGrupo(RelatorioQuantitativoProblemaDTO relatorioQuantitativoProblemaDto) throws Exception {
		return getProblemaDao().listaQuantidadeProblemaPorGrupo(relatorioQuantitativoProblemaDto);
	}
	...
	public Collection<RelatorioQuantitativoProblemaDTO> listaQuantidadeProblemaPorOrigem(RelatorioQuantitativoProblemaDTO relatorioQuantitativoProblemaDto) throws Exception {
		return getProblemaDao().listaQuantidadeProblemaPorOrigem(relatorioQuantitativoProblemaDto);
	}
	...
	public Collection<RelatorioQuantitativoProblemaDTO> listaQuantidadeProblemaPorSolicitante(RelatorioQuantitativoProblemaDTO relatorioQuantitativoProblemaDto) throws Exception {
		return getProblemaDao().listaQuantidadeProblemaPorSolicitante(relatorioQuantitativoProblemaDto);
	}
	...
	public Collection<RelatorioQuantitativoProblemaDTO> listaQuantidadeProblemaPorPrioridade(RelatorioQuantitativoProblemaDTO relatorioQuantitativoProblemaDto) throws Exception {
		return getProblemaDao().listaQuantidadeProblemaPorPrioridade(relatorioQuantitativoProblemaDto);
	}
	...
	public Collection<RelatorioQuantitativoProblemaDTO> listaQuantidadeProblemaPorCategoria(RelatorioQuantitativoProblemaDTO relatorioQuantitativoProblemaDto) throws Exception {
		return getProblemaDao().listaQuantidadeProblemaPorCategoria(relatorioQuantitativoProblemaDto);
	}
	...
	public Collection<RelatorioQuantitativoProblemaDTO> listaQuantidadeProblemaPorProprietario(RelatorioQuantitativoProblemaDTO relatorioQuantitativoProblemaDto) throws Exception {
		return getProblemaDao().listaQuantidadeProblemaPorProprietario(relatorioQuantitativoProblemaDto);
	}
	...
	public Collection<RelatorioQuantitativoProblemaDTO> listaQuantidadeProblemaPorImpacto(RelatorioQuantitativoProblemaDTO relatorioQuantitativoProblemaDto) throws Exception {
		return getProblemaDao().listaQuantidadeProblemaPorImpacto(relatorioQuantitativoProblemaDto);
	}
	...
	public Collection<RelatorioQuantitativoProblemaDTO> listaQuantidadeProblemaPorUrgencia(RelatorioQuantitativoProblemaDTO relatorioQuantitativoProblemaDto) throws Exception {
		return getProblemaDao().listaQuantidadeProblemaPorUrgencia(relatorioQuantitativoProblemaDto);
	}
	...
	public void fechaRelacionamentoProblema(ProblemaDTO problemaDTO, TransactionControler tc){
		try {
			if(problemaDTO != null && problemaDTO.getFecharItensRelacionados() != null){
				...
				/*this.fechaItemConfiguracaoVinculadoProblema(problemaDTO, tc);
					this.fechaErroConhecidodoProblema(problemaDTO, tc);*/
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public void fechaSolicitacaoServicoVinculadaProblema(ProblemaDTO problemaDTO, TransactionControler tc){
		try {
			SolicitacaoServicoProblemaDao solicitacaoServicoProblemaDao = new SolicitacaoServicoProblemaDao();
			solicitacaoServicoProblemaDao.setTransactionControler(tc);
			List<SolicitacaoServicoDTO> listSolicitacaoServicoDTO  = (List<SolicitacaoServicoDTO>) solicitacaoServicoProblemaDao.listSolicitacaoServicoByIdProblema(problemaDTO.getIdProblema().intValue());
	...
	public void updateTimeAction(Integer idGrupoRedirect, Integer idPrioridadeRedirect, Integer idProblema) throws ServiceException, LogicException {
		...
		try {
			...
			if(problemaAuxDto.getIdGrupoAtual() != null){
				//return;
			}
			...
			OcorrenciaProblemaServiceEjb.create(problemaDto, null, strOcorr, OrigemOcorrencia.OUTROS, CategoriaOcorrencia.Atualizacao, null, CategoriaOcorrencia.Atualizacao.getDescricao(), usuarioDTO.getLogin(), 0, justificativaDto, tc);
