...
public class InformacoesContrato extends AjaxFormAction {
	...
	public void imprimirConteudoDiv(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String strImprimir = "";
		// strImprimir =
		// geraDadosImpressoesService.geraCabecalhoImpressoesQuestionario(prontuarioDto.getIdContrato(),
		// idEmpresa, usuario.getIdProfissional(), "INFORMAÇÕES DO PACIENTE",
		// UtilDatas.getDataAtual(), true, true);
		// strImprimir = geraDadosImpressoesService.geraCabecalhoImpressoesSimples(idEmpresa);
	...
	public void delete(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			...
			document.executeScript("showServicosContrato('" + getServicoContratoDTO().getIdContrato() + "');");
		} catch (Exception e) {
			e.getStackTrace();
		}
	}
	...
	public void load(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (request.getSession(true).getAttribute("idAdministracaoContrato") != null && request.getSession(true).getAttribute("numeroContratoAdministracaoContrato") != null) {
			/*document.executeScript("document.getElementById('txtDESCLOOKUP_CONTRATOS').value='" + request.getSession(true).getAttribute("numeroContratoAdministracaoContrato").toString() + "'");
			document.executeScript("document.getElementById('idContrato').value=" + (Integer) request.getSession(true).getAttribute("idAdministracaoContrato"));*/
	...
	public void listarSLAsContrato(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if ((col != null && col.size() > 0) || (colVincs != null && colVincs.size() > 0)) {
			...
			for (Iterator it = col.iterator(); it.hasNext();) {
				...
				table += "</td>";
				table += "<td>";
				table += UtilDatas.dateToSTR(acordoNivelServicoDTO.getDataInicio());
				table += "</td>";
				table += "<td>";
				table += UtilStrings.nullToVazio(UtilDatas.dateToSTR(acordoNivelServicoDTO.getDataFim()));
				table += "</td>";
				table += "<td>";
				table += UtilStrings.nullToVazio(UtilDatas.dateToSTR(acordoNivelServicoDTO.getAvaliarEm()));
				table += "</td>";
				table += "<td>";
				table += "<img src='" + br.com.citframework.util.Constantes.getValue("SERVER_ADDRESS") + br.com.citframework.util.Constantes.getValue("CONTEXTO_APLICACAO")
			...
			for (Iterator it = colVincs.iterator(); it.hasNext();) {
				...
				table += "</td>";
				table += "<td>";
				table += UtilDatas.dateToSTR(acordoServicoContratoDTO.getDataInicio());
				table += "</td>";
				table += "<td>";
				table += UtilStrings.nullToVazio(UtilDatas.dateToSTR(dataFim));
				table += "</td>";
				table += "<td>";
				table += UtilStrings.nullToVazio(UtilDatas.dateToSTR(acordoNivelServicoDTO.getAvaliarEm()));
				table += "</td>";
	...
	public void listarValoresContrato(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (col != null && col.size() > 0) {
			for (Iterator it = col.iterator(); it.hasNext();) {
				...
				table += "<td>" + UtilDatas.dateToSTR(valoresServicoContratoDTO.getDataInicio()) + "</td>";
				table += "<td>" + (valoresServicoContratoDTO.getDataFim() == null ? "" : UtilDatas.dateToSTR(valoresServicoContratoDTO.getDataFim())) + "</td>";
				table += "</tr>";
			}
		} else {
			table += "<tr>";
		...
		if (colOSs != null && colOSs.size() > 0) {
			for (Iterator itOs = colOSs.iterator(); itOs.hasNext();) {
				...
				if (colOSFilho != null && colOSFilho.size() > 0) {
					...
					// if(funcaoListarOs == OSDTO.TODAS || funcaoListarOs ==
					// OSDTO.EM_CRIACAO || funcaoListarOs == OSDTO.SOLICITADA ||
					// funcaoListarOs == OSDTO.APROVADA || funcaoListarOs ==
					// OSDTO.CANCELADA){
					// strTable += this.montaTabelaContratoOs(colOSFilho, osDto,
					// coricone, nomeMoeda, funcaoListarOs);
					// }else if(funcaoListarOs == OSDTO.AUTORIZADA){
					// strTable += this.montaTabelaContratoOs(colOSFilho, osDto,
					// coricone, nomeMoeda, funcaoListarOs);
					// }
				}
	...
	public boolean verificaSituacaoOSValida(OSDTO osDto) throws Exception {
		...
		if (osDto != null) {
			...
			if (servicoDto != null && servicoContratoDTO != null) {
				...
				// if(servicoDto.getIdTipoServico() != TipoServicoDTO.SUPORTE){
				// boolean flag = osService.retornaRegistroOsPai(osDto);
				// return flag;
				// }else if(servicoDto.getIdTipoServico() ==
				// TipoServicoDTO.SUPORTE){
	...
	public String montaTabelaContratoOs(Collection colOSFilho, OSDTO osDto, String coricone, String nomeMoeda, Integer funcaoListarOs, HttpServletRequest request) throws ServiceException, Exception {
		...
		if (funcaoListarOs == OSDTO.APROVADA) {
			boolean flag = verificaSituacaoOSValida(osDto);
			if (!flag) {
				return strTable;
			}
		}

		// if(funcaoListarOs == OSDTO.CANCELADA && osDto.getIdOSPai() == null){
		// return strTable;
		// }else if(funcaoListarOs == OSDTO.CANCELADA){
		// osDto.setIdOSPai(null);
		// }
		...
		// strTable += "<img style='margin: 1px' src='" +
		// Constantes.getValue("SERVER_ADDRESS") +
		// Constantes.getValue("CONTEXTO_APLICACAO")
		// +
		// "/imagens/plus.png' border='0' title='Duplicar OS' onclick='duplicarOS(\""
		// + osDto.getIdOS() + "\")' style='cursor:pointer'/>";
		...
		if (colOSFilho != null && colOSFilho.size() > 0) {
			...
			for (Iterator itOsFilho = colOSFilho.iterator(); itOsFilho.hasNext();) {
				...
				List<OSDTO> colAtividadeOs = (List<OSDTO>) osService.listAtividadePeridodicaByIdos(osFilhoDto.getIdOS());
				...
				// Constantes.getValue("SERVER_ADDRESS") +
				// Constantes.getValue("CONTEXTO_APLICACAO") +
				// "/imagens/calendar.png' border='0' title='"
				// + UtilI18N.internacionaliza(request,
				// "contrato.agendamentoOS") + "' onclick='agendamentoOS(\"" +
				// osFilhoDto.getIdOS() + "\")' style='cursor:pointer'/>";
				// } else {
				// coricone = "<img src='" +
				// Constantes.getValue("SERVER_ADDRESS") +
				// Constantes.getValue("CONTEXTO_APLICACAO") +
				// "/imagens/calendar-empty.png' border='0' title='"
				// + UtilI18N.internacionaliza(request,
				// "contrato.agendamentoOS") + "' onclick='alert(\"" +
				// UtilI18N.internacionaliza(request,
				// "contrato.naoExisteAgendamentoOS")
				// + "\")' style='cursor:pointer'/>";
				// }
	...
	public String tabelaRetornaColunaOs(OSDTO osDto, Integer funcaoListarOs, HttpServletRequest request) throws ServiceException, Exception {
		...
		if (osDto != null && osDto.getDataInicio() != null) {
			strTable += UtilDatas.dateToSTR(osDto.getDataInicio());
		}
...
//		strTable += "<tr style='text-align: center;'>";
		...
		if (osDto != null && osDto.getDataFim() != null) {
			if (osDto.getSituacaoOS().equals(OSDTO.EM_EXECUCAO)) {
				if (osDto.getDataFim().before(UtilDatas.getDataAtual())) {
					...
					//strTable += "</td><td style='text-align: center;'>";
				}
			}
		}
		strTable += "<td style='text-align: center;'>";
		...
		if (osDto != null && osDto.getDataFim() != null) {
			if (atrasado) {
				strTable += imagemGifAlerta+"<font color='red'>  " + UtilDatas.dateToSTR(osDto.getDataFim()) + "</font>";
			} else {
				strTable += UtilDatas.dateToSTR(osDto.getDataFim());
			}
		}
...
//		strTable += "</td>";
		...
		if (osDto != null && osDto.getIdOSPai() == null) {
			...
			// strTable += osDto.getQuantidadeGlosasAnterior();
			// } else {
			// strTable += "--";
			// }
	...
	public void listarFaturaContrato(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (colOSs != null && colOSs.size() > 0) {
			for (Iterator itOs = colOSs.iterator(); itOs.hasNext();) {
				...
				strTable += "</td>";
				strTable += "<td>";
				strTable += UtilDatas.dateToSTR(faturaDto.getDataInicial());
				strTable += "</td>";
				strTable += "<td>";
				strTable += UtilDatas.dateToSTR(faturaDto.getDataFinal());
				strTable += "</td>";
				strTable += "<td>";
				strTable += UtilDatas.dateToSTR(faturaDto.getDataCriacao());
				strTable += "</td>";
				strTable += "<td>";
				strTable += nomeMoeda;
				strTable += "</td>";
	...
	public void imprimirRAOSContrato(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (osDto != null) {
			...
			strBuffer += "<br>";
			setContLinha();
			strBuffer += "<b>Per&iacute;odo:</b>" + UtilDatas.dateToSTR(osDto.getDataInicio()) + " " + "a " + UtilDatas.dateToSTR(osDto.getDataFim()) + "<br>";
			setContLinha();
			strBuffer += "</td>";
			strBuffer += "</tr>";
			...
			if (colAtividades != null) {
				...
				for (Iterator it = colAtividades.iterator(); it.hasNext();) {
					...
					if (atividadesOSDto.getGlosaAtividade() != null) {
						...
						strBuffer += "<td style='font:12px bold  arial,sans-serif;border:1px solid black;text-align:center'>";
						// strBuffer += UtilFormatacao.formatDouble(realizado,
						// 2);
						strBuffer += "0,00";
						strBuffer += "</td>";
					}
		...
		Thread.sleep(50);
		{
			...
			for (int i2 = 1; i2 < total; i2++) {
				...
				over.showText("Pagina: " + i2 + " de " + (total - 1) + "     impresso em: " + UtilDatas.formatTimestamp(UtilDatas.getDataHoraAtual()) + " por: " + usuarioImpressao);
				over.endText();
				over.stroke();
			}
			stamper.close();
	...
	public void agendamentoOS(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		StringBuffer tableInterna = new StringBuffer();
		StringBuffer tableTotalOs = new StringBuffer();
				...
				if (colEventos != null && colEventos.size() > 0) {
					...
					for (EventoDTO eventoDto : colEventos) {
						...
						if (eventoDto.getClassName().equalsIgnoreCase("suspenso")) {
							contSuspenso++;
							situacao = "Registrado";
							titulo = UtilDatas.dateToSTR(execucaoAtividadePeriodicaDTO.getDataExecucao()) + " - " + execucaoAtividadePeriodicaDTO.getHoraExecucao();
						}
						...
						tableInterna.append("</td>");
						tableInterna.append("<td style='font:12px bold  arial,sans-serif;border:1px solid #B3B3B3; height:20px' align='center'>");
						tableInterna.append(UtilDatas.dateToSTR(eventoDto.getData()) + " - " + eventoDto.getHoraInicio());
						tableInterna.append("</td>");
						tableInterna.append("<td style='font:12px bold  arial,sans-serif;border:1px solid #B3B3B3; height:20px' align='center'>");
		...
		/*
		 * tableTotalOs.append("<tr>"); tableTotalOs.append( "<td style='font:12px' class='linhaSubtituloGridOs' colspan='5'>");
		 */
		...
		/*
		 * tableTotalOs.append("</td>"); tableTotalOs.append("</tr>");
		 */
		...
		/* document.executeScript("fecharAgendamentoOS()"); */
	}
	...
	public void imprimirOSContrato(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (osDto != null) {
			...
			setContLinha();
			strBuffer += "Execução Início: " + (osDto.getDataInicio() != null ? UtilDatas.dateToSTR(osDto.getDataInicio()) : "") + "<br>";
			setContLinha();
			strBuffer += "Execução Final: " + (osDto.getDataFim() != null ? UtilDatas.dateToSTR(osDto.getDataInicio()) : "") + "<br>";
			setContLinha();
			strBuffer += "</td>";
			strBuffer += "</tr>";
			strBuffer += "</table><br><br>";
	...
	public void imprimirFaturaContrato(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		setContLinha();
		strBuffer += "In&iacute;cio: " + UtilDatas.dateToSTR(faturaDto.getDataInicial()) + "<br>";
		setContLinha();
		strBuffer += "Final: " + UtilDatas.dateToSTR(faturaDto.getDataFinal()) + "<br>";
		setContLinha();
		strBuffer += "</td>";
		strBuffer += "</tr>";
		strBuffer += "</table><br>";
		...
		// strBuffer += "<tr>";
		/*
		 * strBuffer += "<td style='border:1px solid black; width:40%; font:12px arial,sans-serif;'' width='40%'>" ; strBuffer += "Saldo Total Previsto"; strBuffer += "</td>";
		 */
		// strBuffer +=
		// "<td style='border:1px solid black; text-align=right; width:20%; font:12px arial,sans-serif;'' width='20%' ALIGN=RIGHT>";
		...
		if (faturaDto.getSaldoPrevisto() != null) {
			// strBuffer +=
			// UtilFormatacao.formatDouble(faturaDto.getSaldoPrevisto(), 2) +
			// "<br>";
			setContLinha();
		} /*
		 * else { // strBuffer += "0,00"; }
		...
		/*
		 * strBuffer += "</td>"; strBuffer += "<td style='border:1px solid black; text-align=right; width:30%; font:12px arial,sans-serif;'' width='30%' ALIGN=RIGHT>" ; strBuffer += ""; strBuffer +=
		 * "</td>"; strBuffer += "</tr>";
		 */
		...
		if (colQuestHist != null) {
			...
			for (Iterator it = colQuestHist.iterator(); it.hasNext();) {
				...
				if (col != null && col.size() > 0) {
					...
					if (!isPerfilUsuarioLogadoInCollection(colPerfisAssociados, usuario)) {
						continue;
					}
				...
				strTable += "<td class='tdPontilhada'>";
				strTable += UtilDatas.dateToSTR(contratoQuestDTO.getDataQuestionario());
				strTable += "</td>";
				strTable += "<td class='tdPontilhada'>";
	...
	public void imprimirRelatorioOrdemServico(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// ArrayList<AcordoNivelServicoDTO> colAcordosServicoContrato =
		// (ArrayList<AcordoNivelServicoDTO>)
		// acordoNivelServicoService.findByIdServicoContrato(osDto.getIdServicoContrato());
		...
		Date dt = new Date();
		String strCompl = "" + dt.getTime();
		String caminhoJasper = CITCorporeUtil.caminho_real_app + Constantes.getValue("CAMINHO_RELATORIOS") + "RelatorioOrdemServico.jasper";
		...
		parametros.put("ano", converterData(contratoDto.getDataContrato()));
		...
		JasperExportManager.exportReportToPdfFile(print, diretorioReceita + "/RelatorioOrdemServico" + strCompl + "_" + user.getIdUsuario() + ".pdf");
		...
		document.executeScript("window.open('" + Constantes.getValue("SERVER_ADDRESS") + Constantes.getValue("CONTEXTO_APLICACAO") + "/printPDF/printPDF.jsp?url=" + diretorioRelativoOS
				+ "/RelatorioOrdemServico" + strCompl + "_" + user.getIdUsuario() + ".pdf')");
		document.getJanelaPopupById("JANELA_AGUARDE_MENU").hide();

	}

	public void imprimirRelatorioRAOrdemServicoContrato(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (colAtividades != null) {
			for (Iterator it = colAtividades.iterator(); it.hasNext();) {
				AtividadesOSDTO atividade = (AtividadesOSDTO) it.next();
		...
		Date dt = new Date();
		String strCompl = "" + dt.getTime();
		String caminhoJasper = CITCorporeUtil.caminho_real_app + Constantes.getValue("CAMINHO_RELATORIOS") + "RelatorioRAOrdemServico.jasper";
		String caminhoSubRelatorioJasper = CITCorporeUtil.caminho_real_app + Constantes.getValue("CAMINHO_RELATORIOS");
		String diretorioReceita = CITCorporeUtil.caminho_real_app + "/tempFiles";
		String diretorioRelativoOS = Constantes.getValue("SERVER_ADDRESS") + Constantes.getValue("CONTEXTO_APLICACAO") + "/tempFiles";
		...
		JasperPrint print = JasperFillManager.fillReport(caminhoJasper, parametros, dataSource);
		JasperExportManager.exportReportToPdfFile(print, diretorioReceita + "/RelatorioRAOrdemServico" + strCompl + "_" + user.getIdUsuario() + ".pdf");

		document.executeScript("window.open('" + Constantes.getValue("SERVER_ADDRESS") + Constantes.getValue("CONTEXTO_APLICACAO") + "/printPDF/printPDF.jsp?url=" + diretorioRelativoOS
				+ "/RelatorioRAOrdemServico" + strCompl + "_" + user.getIdUsuario() + ".pdf')");
		document.getJanelaPopupById("JANELA_AGUARDE_MENU").hide();
	...
	public void imprimirRelatorioFaturaContratoXls(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		exporter.setParameter(JRXlsExporterParameter.OUTPUT_FILE_NAME, diretorioReceita + "/RelatorioFaturaContratoXls" + strCompl + "_" + user.getIdUsuario() + ".xls");
	...
	public void imprimirRelatorioRAOrdemServicoContratoXls(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		JasperDesign desenho = JRXmlLoader.load(CITCorporeUtil.caminho_real_app + Constantes.getValue("CAMINHO_RELATORIOS") + "RelatorioRAOrdemServicoXls.jrxml");
		...
		exporter.setParameter(JRXlsExporterParameter.OUTPUT_FILE_NAME, diretorioReceita + "/RelatorioRAOrdemServicoXls" + strCompl + "_" + user.getIdUsuario() + ".xls");

		exporter.exportReport();

		document.executeScript("window.open('" + Constantes.getValue("SERVER_ADDRESS") + Constantes.getValue("CONTEXTO_APLICACAO") + "/printPDF/printPDF.jsp?url=" + diretorioRelativoOS
				+ "/RelatorioRAOrdemServicoXls" + strCompl + "_" + user.getIdUsuario() + ".xls')");
		document.getJanelaPopupById("JANELA_AGUARDE_MENU").hide();

	}

	public void imprimirRelatorioOrdemServicoXls(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// acordoNivelServicoService.findByIdServicoContrato(osDto.getIdServicoContrato());
		Collection<ComplexidadeDTO> listaComplexidadePorContrato = null;
		Collection<AcordoServicoContratoDTO> colAcordosServicoContrato = null;
		Collection colAtividades = null;
		if (osDto.getIdServicoContrato() != null) {
			colAcordosServicoContrato = acordoNivelServicoService.consultaPorIdServicoContrato(osDto.getIdServicoContrato());
			listaComplexidadePorContrato = contratoService.listaComplexidadePorContrato(osDto.getIdServicoContrato());
		...
		JasperDesign desenho = JRXmlLoader.load(CITCorporeUtil.caminho_real_app + Constantes.getValue("CAMINHO_RELATORIOS") + "RelatorioOrdemServicoXls.jrxml");
		...
		exporter.setParameter(JRXlsExporterParameter.IS_REMOVE_EMPTY_SPACE_BETWEEN_ROWS, Boolean.TRUE);
		exporter.setParameter(JRXlsExporterParameter.IS_WHITE_PAGE_BACKGROUND, Boolean.TRUE);
		exporter.setParameter(JRXlsExporterParameter.OUTPUT_FILE_NAME, diretorioReceita + "/RelatorioOrdemServicoXls" + strCompl + "_" + user.getIdUsuario() + ".xls");

		exporter.exportReport();

		document.executeScript("window.open('" + Constantes.getValue("SERVER_ADDRESS") + Constantes.getValue("CONTEXTO_APLICACAO") + "/printPDF/printPDF.jsp?url=" + diretorioRelativoOS
				+ "/RelatorioOrdemServicoXls" + strCompl + "_" + user.getIdUsuario() + ".xls')");
		document.getJanelaPopupById("JANELA_AGUARDE_MENU").hide();
