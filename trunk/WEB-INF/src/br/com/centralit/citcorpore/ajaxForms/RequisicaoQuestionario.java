...
public class RequisicaoQuestionario extends SolicitacaoServicoQuestionario {
	...
	public void load(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		RequisicaoQuestionarioDTO requisicaoQuestionarioDTO = (RequisicaoQuestionarioDTO) document.getBean();
			...
			if (!arquivoMultimidia.getCaminhoArquivo().startsWith("IDCITGED=")){
				...
				if (GED_INTERNO.equalsIgnoreCase("S") && !"S".equalsIgnoreCase(prontuarioGedInternoBancoDados)){ //Se utiliza GED interno
					if (controleGEDDTO != null){
						...
						if (requisicaoQuestionarioDTO.getColCertificados() != null && requisicaoQuestionarioDTO.getColCertificados().size() > 0){
							int ordem = 1;
							CITAssinadorDigital d = new CITAssinadorDigital();
							d.inicializar();				
							for(Iterator itCert = requisicaoQuestionarioDTO.getColCertificados().iterator(); itCert.hasNext();){
		...
		this.createQuestionario(document, request, response, requisicaoQuestionarioDTO);
	}	
	
	public void createQuestionario(DocumentHTML document, HttpServletRequest request, HttpServletResponse response, RequisicaoQuestionarioDTO requisicaoQuestionarioDTO) throws ServiceException, Exception{
		UsuarioDTO usrDto = (UsuarioDTO) br.com.centralit.citcorpore.util.WebUtil.getUsuario(request);
	    RequisicaoQuestionarioService requisicaoQuestionarioService = (RequisicaoQuestionarioService) ServiceLocator.getInstance().getService(RequisicaoQuestionarioService.class, WebUtil.getUsuarioSistema(request));	    
			...
			if (vCertDigDto.getInfoCertificadoDigital() == null){
				...
				strTable += "<tr>";
					...
					strTable += "<td>";
						...
						strTable += "<b>" + vCertDigDto.getInfoCertificadoDigital().getPais() + "</b> Tipo: <b>" + vCertDigDto.getInfoCertificadoDigital().getTipo() + "</b> Validade entre: <b>" + UtilDatas.dateToSTR(vCertDigDto.getInfoCertificadoDigital().getDataInicioValidade()) + " a " + UtilDatas.dateToSTR(vCertDigDto.getInfoCertificadoDigital().getDataFimValidade()) + "</b><br>";
	...
	public String geraDadosFormulario(Integer idEmpresa, RequisicaoQuestionarioDTO contQuestBean, boolean geraCabecalho, boolean imprimeDemaisDados, boolean imprimeLogo) throws Exception {		
		...
		RequisicaoQuestionarioDTO contQuestPesqBean = (RequisicaoQuestionarioDTO) contratoQuestionariosService.restore(contQuestBean);
		...
		if (questionario != null){
			...
			for(Iterator it = colGrupos.iterator(); it.hasNext();){
				 ...
				 Collection colRetorno = grupoQuestionarioService.geraImpressaoFormatadaHTML(grupoQuestDto.getColQuestoes(), contQuestPesqBean.getDataQuestionario(), contQuestPesqBean.getIdRequisicao(), contQuestPesqBean.getIdResponsavel());
	...
	public String geraDadosFormularioSemAssinatura(Integer idEmpresa, RequisicaoQuestionarioDTO contQuestBean, boolean geraCabecalho, boolean imprimeDemaisDados, boolean imprimeLogo) throws Exception {		
		ContratoQuestionariosService contratoQuestionariosService = (ContratoQuestionariosService) ServiceLocator.getInstance().getService(ContratoQuestionariosService.class, null);
		RequisicaoQuestionarioDTO contQuestPesqBean = (RequisicaoQuestionarioDTO) contratoQuestionariosService.restore(contQuestBean);
	...
	public String gerarHistoricoGeralPacienteHTML(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (pesQuestBean.getOrdemHistorico() != null && pesQuestBean.getOrdemHistorico().equalsIgnoreCase("D")){
			if (pesQuestBean.getAba() == null || pesQuestBean.getAba().trim().equalsIgnoreCase("")){
				...
				col = contratoQuestionariosService.listByIdContratoAndAba(pesQuestBean.getIdRequisicao(), pesQuestBean.getAba());
			}			
		}else{
			if (pesQuestBean.getAba() == null || pesQuestBean.getAba().trim().equalsIgnoreCase("")){
				col = contratoQuestionariosService.listByIdContrato(pesQuestBean.getIdRequisicao());
			}else{
				col = contratoQuestionariosService.listByIdContratoAndAbaOrdemCrescente(pesQuestBean.getIdRequisicao(), pesQuestBean.getAba());
			}
		}
		...
		if (col != null){
			for(Iterator it = col.iterator(); it.hasNext();){
				RequisicaoQuestionarioDTO contratoQuestionariosAux = (RequisicaoQuestionarioDTO)it.next();
					...
					if (isPerfilUsuarioLogadoInCollection(colPerfisAssociados, usuario)){ //Se tiver acesso a aquela ABA, então gera historico.
						...
						if (contratoQuestionariosAux.getConteudoImpresso() != null &&
							...
							try{
								...
								contratoQuestionariosService.updateConteudoImpresso(contratoQuestionariosAux.getIdRequisicaoQuestionario(), conteudoImp);
							}catch (Exception e) {
								e.printStackTrace();
							}
							str += conteudoImp;
	...
	public void gerarHistoricoGeralPaciente(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		for (int i = 1; i < total; i++) {
			...
			over.showText("Pagina: " + i + " de " + (total - 1) + "     prontuário impresso em: " + UtilDatas.formatTimestamp(UtilDatas.getDataHoraAtual()) + " por: " + usuarioImpressao);
			over.endText();
	...
	*/
	...
	public void imprimir(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		for (int i = 1; i < total; i++) {
			...
			over.setFontAndSize(bf, 8);
			over.setTextMatrix(30, 30);
			over.showText("Pagina: " + i + " de " + (total - 1) + "      impresso em: " + UtilDatas.formatTimestamp(UtilDatas.getDataHoraAtual()) + "      por: " + usuarioImpressao);
			over.endText();
			//over.setRGBColorStroke(0xFF, 0x00, 0x00);
			//over.setLineWidth(5f);
			//over.ellipse(250, 450, 350, 550);
    ...
    public void visualizarHistoricoCampo(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        RequisicaoQuestionarioDTO contrQuestBean = (RequisicaoQuestionarioDTO) document.getBean();
        	...
        	for(Iterator it = col.iterator(); it.hasNext();){
        		...
        		if (questaoDto.getTipoQuestao().equalsIgnoreCase("D")){ 
        			if (resposta.getRespostaData() != null){
        				strItem += UtilDatas.dateToSTR(resposta.getRespostaData());
        			}
        		}
        		...
        		strTable += "<tr><td>" + UtilDatas.dateToSTR(questaoDto.getDataRegistro()) + "</td><td>" + strItem + "</td></tr>";
        	}
        }
        	...
        	for(Iterator it = col.iterator(); it.hasNext();){
    			...
    			try{
	    			if (questaoDto.getTipoQuestao().equalsIgnoreCase("N")){
	    				...
	    				dados.addValue(objDouble.doubleValue(), "Valor", UtilDatas.dateToSTR(questaoDto.getDataRegistro()));
	    			}
	    			...
	    			if (questaoDto.getTipoQuestao().equalsIgnoreCase("*")){
	    				...
	    				if (objDouble == null)
	    					objDouble = new Double(0);
	    				dados.addValue(objDouble.doubleValue(), "%", UtilDatas.dateToSTR(questaoDto.getDataRegistro()));
	    			...
	    			if (questaoDto.getTipoQuestao().equalsIgnoreCase("1")){
	    				...
	    				if (objDouble == null)
	    					objDouble = new Double(0);
	    				dados.addValue(objDouble.doubleValue(), "Valor 1", UtilDatas.dateToSTR(questaoDto.getDataRegistro()));
	
	    				objDouble = resposta.getRespostaNumero2();
	    				if (objDouble == null)
	    					objDouble = new Double(0);
	    				dados.addValue(objDouble.doubleValue(), "Valor 2", UtilDatas.dateToSTR(questaoDto.getDataRegistro()));
	    			}
