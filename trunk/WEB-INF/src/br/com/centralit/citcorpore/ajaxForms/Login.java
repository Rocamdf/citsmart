...
public class Login extends AjaxFormAction {
	...
	public void load(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(!instalacaoService.isSucessoInstalacao() && request.getSession().getAttribute("passoInstalacao") == null){
			document.executeScript("mostraMensagemInsercao('"
					+ "<label>" + UtilI18N.internacionaliza(request, "instalacao.mensagemInsucesso1") + "</label>"
		...
		if (context.getAttribute("instalacao") == null && request.getSession().getAttribute("passoInstalacao") == null
				&& (request.getSession().getAttribute("locale") == null || request.getSession().getAttribute("locale").equals(""))) {
	...
	public void Login_onsave(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (login != null) {
			/**
			 * Motivo: Inersão da validação de login
			 * Autor: Flávio.santana
			 * Data/Hora: 05/11/2013 17:30
			 */
		...
		/*
		 * Collection listaUsuario = new ArrayList();
		 * 
		 * listaUsuario = usuarioService.list();
		 */
		...
		if (metodoAutenticacao != null) {
			if (usuarioBean.getStatus().equalsIgnoreCase("A") && login.getSenha().equals(usuarioBean.getSenha())) {
				...
				/* Desenvolvedor: Thiago Matias - Data: 05/11/2013 - Horário: 16:00 - ID Citsmart: 123357 - 
				* Motivo/Comentário: redirecionando para o portal quando o parametro do mesmo estiver habilitado.
				*/
