...
//		String divInfo = "" + UtilI18N.internacionaliza(request, array[0]) + "";
	...
	public void geraListaIps(DocumentHTML document, HttpServletRequest request,
		...
		if (listNetMapDto != null) {
			...
			for (NetMapDTO netMapDTO : listNetMapDto) {
				...
				if (!netMapDTO.okTimeToProcess()){
					dataHoraControle = UtilI18N.internacionaliza(request, "mostrarStatusInventario.proximaVerificacao") + UtilDatas.formatTimestamp(netMapDTO.getDateTimeControlProcessInv());
				}
		...
		String divInfo2 = "" + MonitoraAtivosDiscovery.MENSAGEM_PROCESSAMENTO_COMPL + "";
				...
				for (NetMapDTO netMapAux : listNetMapDto) {
					...
					if (nameOrIp.equalsIgnoreCase(netMapDTO.getIp())){
						...
						if (ipAddr.ping()){
							...
							document.alert(UtilI18N.internacionaliza(request, "mostrarStatusInventario.oHost")+" '" + nameOrIp + "' "+UtilI18N.internacionaliza(request, "mostraStatusIventario.estaAtivo"));
						}else{
							netMapAux.setStatusPing(NetMapDTO.INATIVO);
							document.alert(UtilI18N.internacionaliza(request, "mostrarStatusInventario.oHost")+" '" + nameOrIp + "' "+UtilI18N.internacionaliza(request, "mostraStatusIventario.estaInativo"));
						}
					}
				}
			}
					...
					if (ips.length > 1){
						...
						try{
					        ...
					        do {
					        	...
					        	if (validaIP.trim().equalsIgnoreCase("S")){
						        	final Future<String> runFuture = exService.submit(new RunnableThread(ipAux, validaIP, nativePing), "done");
						        	qtde++;
						        	if (qtde > numThreads){
						        		qtde = 0;
						        		try {
