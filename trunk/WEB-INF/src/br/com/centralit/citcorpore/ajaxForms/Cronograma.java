...
public class Cronograma extends AjaxFormAction {
	...
	public void saveMarcosPag(DocumentHTML document, HttpServletRequest request,
		...
		try {
			...
			Collection colMarcos = getMarcosPag(obj);
			if (colMarcos != null && colMarcos.size() > 0){
		...
		if (colMarcos != null){
			for (Iterator it = colMarcos.iterator(); it.hasNext();){
				...
				nomeMarco = nomeMarco.replaceAll("\"", "");
				String dataPagStr = UtilDatas.dateToSTR(marcoPagamentoPrjDTO.getDataPrevisaoPag());
	...
	public void save(DocumentHTML document, HttpServletRequest request,
		...
		HashMap<String, Object> map = null;
		try {
			map = JSONUtil.convertJsonToMap(cronogramaDTO.getTa(), true);
			Collection obj = (Collection) map.get("TASKS");
			Collection<TarefaLinhaBaseProjetoDTO> colTarefas = getTarefas(obj);
		...
		if (linhaBaseProjetoDTO != null){
			...
			if (colTarefasLnBase != null){
				...
				/*	if(objDepends != null){
					...
					strBuffer.append("{\"id\":" + tarefaLinhaBaseProjetoDTO.getIdTarefaLinhaBaseProjeto() + ",\"name\":\"" + nome + "\",\"code\":\"" + code + "\",\"level\":" + nivel + "," +
	...
	private Collection getTarefas(Collection col){
		if (col != null){
	...
	private Collection getMarcosPag(Collection col){
		if (col != null){
			Collection colRetorno = new ArrayList();
			for (Iterator it = col.iterator(); it.hasNext();){
				HashMap mapItem = (HashMap)it.next(); 
				MarcoPagamentoPrjDTO marcoPagamentoPrjDTO = generateMarcoPagDTO(mapItem);
	...
	private MarcoPagamentoPrjDTO generateMarcoPagDTO(HashMap mapItem){
		if (mapItem == null){
			return null;
		}
		MarcoPagamentoPrjDTO marcoPagamentoPrjDTO = new MarcoPagamentoPrjDTO();
		...
		if (!dataStr.trim().equalsIgnoreCase("")){
			try{
				marcoPagamentoPrjDTO.setDataPrevisaoPag(UtilDatas.strToSQLDate(dataStr));
			}catch(Exception e){}
		}
