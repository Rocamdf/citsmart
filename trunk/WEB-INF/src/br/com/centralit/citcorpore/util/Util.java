...
import br.com.centralit.citcorpore.seguranca.ConexaoBD;
...
public class Util implements Serializable {
	...
	public static Collection existe(String tabela, String condicao,
			Class classe, String nomeCampoId) throws SystemException,
			SQLException {
		...
		if (!tabela.equalsIgnoreCase("")) {
			if (condicao.equalsIgnoreCase("")) {
				...
				sql = STMT_EXISTE.replaceAll("T#", tabela + " WHERE "
						+ condicao);
			}
		}
		try {
			/*
			 * INÍCIO - ALTERAÇÃO DO SQL QUANDO ESTE POSSUI PARÂMETROS DO TIPO
			 * DATA
			 */
			...
			if (!datas.isEmpty()) {
				...
				for (Object d : datas) {
					stmt.setDate(i++,
							new java.sql.Date(formatStringToDate(d.toString())
									.getTime()));
				}
			}
			...
			while (rs.next()) {
				...
				try {
					Object o = null;
					if(constructor != null){
						o = constructor.newInstance(parmsConst);
					}
					col.add(o);
			...
			// }else{
			// e.printStackTrace();
			// throw new SystemException("sis0001", e.getMessage());
			// }
		}
	 ...
	 * @param url
	 */
	public static final void ReencaminhaURL(HttpServletRequest request,
	...
	public static final String getInformacoesDia() {
		...
		retorno = diaSemana + ", " + hoje.get(Calendar.DAY_OF_MONTH) + " de "
				+ nomeMes + " de " + hoje.get(Calendar.YEAR);
		return retorno;
	}
	...
	public static final String encodeHTML(String string) {
		...
		for (int i = 0; i < length; i++) {
			...
			// encoded.append("&quot;");
				...
				encoded.append("&Ccedil;");
			else if (c == 'á' || c == 'Á' || c == 'é' || c == 'É' || c == 'í'
					|| c == 'Í' || c == 'ó' || c == 'Ó' || c == 'ú' || c == 'Ú')
				encoded.append("&" + getLetraCorrespondente(c) + "acute;");
			else if (c == 'â' || c == 'Â' || c == 'ê' || c == 'Ê' || c == 'î'
					|| c == 'Î' || c == 'ô' || c == 'Ô' || c == 'û' || c == 'Û')
	...
	public static String converterTextoInstitucionalHTML(String textoConverter) {
		...
		retorno = retorno.replaceAll("<A href=\"file://",
	 ...
	 * @return
	 */
	public static Date converteStringToDataUtil(String data, String pattern)
	 ...
	 * @return
	 */
	public static java.sql.Date converteStringToDataSQL(String data,
	 ...
	 */
	public static StringBuffer createItemsCombo(Collection col, Class classe,
			String campoId, String campoDesc, int valor) throws SystemException {
		...
		while (it.hasNext()) {
			objeto = it.next();
			try {
				campoId = campoId.substring(0, 1).toUpperCase()
						+ campoId.substring(1);
				campoDesc = campoDesc.substring(0, 1).toUpperCase()
						+ campoDesc.substring(1);
				...
				retorno.append("<option " + aux + " value=\"" + iAux + "\">"
						+ strAux + "</option>\n");
			} catch (SecurityException e) {
				e.printStackTrace();
				throw new SystemException("sis9999", "ERRO: Classe: "
						+ classe.getName() + "Metodo: " + metodo.getName()
						+ " Exc: " + e.getMessage());
	 ...
	 * @throws SystemException
	 */
	public static StringBuffer createItemsComboStrStr(Collection col,
			Class classe, String campoId, String campoDesc, String valor)
			throws SystemException {
		StringBuffer retorno = new StringBuffer();
		Iterator it = col.iterator();
		...
		while (it.hasNext()) {
			...
			try {
				...
				retorno.append("<option " + aux + " value=\"" + strAux1 + "\">"
						+ strAux2 + "</option>\n");
			} catch (SecurityException e) {
				e.printStackTrace();
				throw new SystemException("sis9999", "ERRO: Classe: "
	...
	public static String formatHora(String horaPar) {
		String newHora = "";
		if (horaPar.length() == 3) {
			newHora = "0" + horaPar.substring(0, 1) + ":"
					+ horaPar.substring(1, 3);
		} else {
	 ...
	 * @return
	 */
	public static String getStringRequest(HttpServletRequest request,
	...
	public static int getLenStringRequest(HttpServletRequest request,
			String identificador) {
		String aux = (String) request.getAttribute(identificador);
		if (aux == null) { // Se getParameter for nulo, tenta pegar em
							// getAttribute.
	...
	public static boolean horarioExisteEmCollecao(String horaInicial,
			String horaFinal, Collection col) {
		if (col == null) {
			return false;
		}
		if (col.isEmpty()) {
		...
		int horaInicialInt = Integer.parseInt("0"
				+ Util.transformHora(horaInicial.trim()));
		int horaFinalInt = Integer.parseInt("0"
				+ Util.transformHora(horaFinal.trim()));
	...
	public static String generateHTMLHidden(HttpServletRequest request,
			Collection colIds) {
		String acum = "";
		String nome = "";
		...
		while (itAux.hasNext()) {
			...
			acum = acum
					+ Util.generateHTMLHidden(nome, Util.nullPorString(Util
							.getStringRequest(request, nome)));
		}
		return acum;
	}

	public static String generateHTMLOptions(String strValores, String charSep,
			String defaultValue, String defaultStr) {
		String[] auxStr = strValores.split(charSep);
		String saida = "";
		for (int i = 0; i < auxStr.length; i++) {
			if (!Util.nullPorString(auxStr[i]).trim().equalsIgnoreCase("")) {
				saida = saida + "<option value=\"" + i + "\">" + auxStr[i]
						+ "</option>\n";
			}
		}
		if (saida.equalsIgnoreCase("")) {
			saida = "<option value=\"" + defaultValue + "\">" + defaultStr
					+ "</option>\n";
		}
		return saida;
	}
	...
	public static String formataNumero(String sValue, String sMask) {
		...
		while (i < tamCampo) {
			if (i2 < sMask.length()) {
				bolMask = ((sMask.charAt(i2) == '-')
						|| (sMask.charAt(i2) == '.')
						|| (sMask.charAt(i2) == '/') || (sMask.charAt(i2) == ':'));
				bolMask = bolMask
						|| ((sMask.charAt(i2) == '(')
								|| (sMask.charAt(i2) == ')') || (sMask
								.charAt(i2) == ' '));
				if (bolMask) {
	...
	public static String retiraMascara(String value) {
		...
		while (i < value.length()) {
			bolMask = ((value.charAt(i) == '-') || (value.charAt(i) == '.')
	...
	public static String calculaIdade(Date dDataNasc, String type, Date ateData) {
		...
		if ((mesHoje < mesDataParm)
				|| ((mesHoje == mesDataParm) && (diaHoje < diaDataParm))) {
	...
	public static void verificaValorInexistente(Object[] array, int indice,
			String msgErro) throws BusinessException {
	...
	public static Date subtraiDiaData(Date dataInicio, int numDias) {
		numDias = numDias*-1;
		Calendar cal = Calendar.getInstance();
		cal.setTime(dataInicio);
	...
	public static String getStringRequestValue(HttpServletRequest request,
			String identificador, String valor, String tipoSel) {
		String aux = (String) request.getAttribute(identificador);
		if (aux == null) { // Se getParameter for nulo, tenta pegar em
							// getAttribute.
			aux = (String) request.getParameter(identificador);
	...
	public static boolean isHoraValida(String hora) {
		...
		double horas = new Double(horaParm.substring(0, 2));
		double minutos = new Double(horaParm.substring(tam - 2, tam));
		return horas <= 23.0 && minutos <= 59.0;
	}

	public static double getHoraDbl(String hora) throws Exception {
	...
	public static String getHoraFmtStr(double hora) {
		double horaParm = hora;
		boolean bNegativo = horaParm < 0;
		if (bNegativo)
			horaParm = horaParm * -1;
	...
	public static java.util.Date getData(int dia, int mes, int ano)
	...
	public static int getDiaSemana(Date dataConsulta) throws LogicException {
	...
	public static int getHora(double hora) {
		double horaParm = hora;
		if (horaParm < 0)
			horaParm = horaParm * -1;
		long s = Math.round(horaParm * 3600);
	...
	public static int getMinuto(double hora) {
		double horaParm = hora;
		if (horaParm < 0)
			horaParm = horaParm * -1;
		long s = Math.round(horaParm * 3600);
	...
	public static double calculaDuracao(String horaInicialStr,
			String horaFinalStr) throws Exception {
		double horaInicial = Util.getHoraDbl(horaInicialStr);
		double horaFinal = Util.getHoraDbl(horaFinalStr);
		if (horaFinal < horaInicial)
			throw new Exception("Hora final é menor que hora a inicial");
		double duracao = horaFinal - horaInicial;
	...
	public static double calculaDuracaoEmMinutos(Date dataInicial,
			String horaInicialStr, Date dataFinal, String horaFinalStr)
			throws Exception {
		double horaInicial = Util.getHoraDbl(horaInicialStr);
		double horaFinal = Util.getHoraDbl(horaFinalStr);
		double duracao = 0.0;
	...
	public static byte[] gerarHash(String senha, String algoritmo) {
		try {
	 ...
	 */
	public static String tratarAspasSimples(String valor) {
	 ...
	 * @return valor tratado.
	 */
	public static String retiraBarraInvertida(String valor) {
		if (valor != null && !StringUtils.isEmpty(valor)) {
			if (StringUtils.contains(valor, "\\")) {
				return StringUtils.replace(valor, "\\", "");
			}else {
	...
	/*
	 * Método que recebe uma String no formato informado e retorna uma nova
	 * String também no formato informado.
	 * 
	...
	public static String converteFormato(String stringISO,
			String encodingEntrada, String encodingSaida)
			throws UnsupportedEncodingException {
		byte[] bISO = stringISO.getBytes(encodingEntrada);
		String s = new String(bISO, encodingSaida);
		return s;
	}
	
	public static XMLGregorianCalendar asXMLGregorianCalendar(java.sql.Timestamp ts) {  
		if(ts == null)  
			return null;  
		...
		XMLGregorianCalendar xmlGregorianCalendar = df.newXMLGregorianCalendar(dateFormat.format(ts));  
