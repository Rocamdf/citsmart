...
public class MonitoraAtivosDiscovery extends Thread {
	public static Semaphore performanceDataSemaphoreAtivosDiscovery = new Semaphore(1);
	public static int PORTA_AGENTE_DOTNET = 7103;
	public static String PORTA_SNMP = "161" ;
	public static String DIRETORIO_ARQUIVOS_INVENTARIO = CITCorporeUtil.caminho_real_app + "/tempInventario";
	...
	public void run() {
		...
		try{
			//Deleta arquivos que ainda existam na pasta.
		...
		if (!CITCorporeUtil.START_MODE_INVENTORY){
			System.out.println("CITSMART --> Processo de Inventário Desativado... Ver arquivo de Configuração...");
		...
		while (true) {
			if (!iniciouDiscovery){
				INVENTARIO_PROCESSAMENTO_ATIVO = ParametroUtil.getValorParametroCitSmartHashMap(ParametroSistema.INVENTARIO_PROCESSAMENTO_ATIVO, null);
				if (INVENTARIO_PROCESSAMENTO_ATIVO != null && INVENTARIO_PROCESSAMENTO_ATIVO.equalsIgnoreCase("N")){
					MENSAGEM_PROCESSAMENTO_COMPL = "";
					MENSAGEM_PROCESSAMENTO = "mostrarStatusInventario.processamentoInventarioDesativado";
					try {
	...
	public void processa() throws Exception {
		...
		//
		List listaHostNamesIPProc = new ArrayList();
		performanceDataSemaphoreAtivosDiscovery.acquireUninterruptibly();
		...
		try {
			...
			if (ipServidorInv == null){
				MENSAGEM_PROCESSAMENTO_COMPL = "";
				MENSAGEM_PROCESSAMENTO = "IP DO SERVIDOR DE INVENTARIO NÃO CONFIGURADO! ENTRE EM PARAMETROS E CONFIGURE!";
				performanceDataSemaphoreAtivosDiscovery.release();
				return;
			}
			*/
			String INVENTARIO_PROCESSAMENTO_ATIVO = ParametroUtil.getValorParametroCitSmartHashMap(ParametroSistema.INVENTARIO_PROCESSAMENTO_ATIVO, "N");
			...
			if (listNetMapDto != null) {
				...
				for (NetMapDTO netMapDTO : listNetMapDto) {
					...
					if (netMapDTO.getStatusPing() == null || netMapDTO.getStatusPing().equalsIgnoreCase(NetMapDTO.INDEFINIDO)){
						table += "<td onclick='fazPing(\"" + nameOrIp + "\")' style='border:1px solid black; cursor:pointer' title='<i18n:message key=\"mostrarStatusInventario.clickAquiParaAtaulizarStatus\"/>'><img src='" + Constantes.getValue("CONTEXTO_APLICACAO") + "/imagens/bolacinza2.png'/></td>";
					}else if (netMapDTO.getStatusPing() != null && netMapDTO.getStatusPing().equalsIgnoreCase(NetMapDTO.ATIVO)){
						table += "<td onclick='fazPing(\"" + nameOrIp + "\")' style='border:1px solid black; cursor:pointer' title='<i18n:message key=\"mostrarStatusInventario.clickAquiParaAtaulizarStatus\"/>'><img src='" + Constantes.getValue("CONTEXTO_APLICACAO") + "/imagens/bolaverde.png'/></td>";
					}else if (netMapDTO.getStatusPing() != null && netMapDTO.getStatusPing().equalsIgnoreCase(NetMapDTO.INATIVO)){
						table += "<td onclick='fazPing(\"" + nameOrIp + "\")' style='border:1px solid black; cursor:pointer' title='<i18n:message key=\"mostrarStatusInventario.clickAquiParaAtaulizarStatus\"/>'><img src='" + Constantes.getValue("CONTEXTO_APLICACAO") + "/imagens/bolavermelha.png'/></td>";					
					}					
					table += "<td style='border:1px solid black'>" + nameOrIp + "</td><td style='border:1px solid black'><button type='button' onclick='inventarioAgora(\"" + nameOrIp + "\")'><i18n:message key=\"mostrarStatusInventario.numeroThreadsDiscovery\"/></button></td><td style='border:1px solid black'>" + dataInv + "</td>" + td + "</tr>";
				}
				table += "</table>";
					...
					if ((netMapDTO.isNovoIC() || netMapDTO.isForce()) && netMapDTO.okTimeToProcess()){ //Primeiro prioriza os ICS novos.
						...
						if (ipAddr.ping()){
							...
							try{
								...
								final Future<String> runFuture = exService.submit(new RunnableThread(t), "done");
								qtde++;
							}catch(Exception e){
								e.printStackTrace();
							}finally{
	...
	public static void main(String[] args) {
		...
		/*
		SimpleDateFormat spd = new SimpleDateFormat("yyyyMMdd");
		String dataAtualInv = spd.format(UtilDatas.getDataAtual()).trim();
		System.out.println("INICIANDO...");
		MonitoraAtivosDiscovery monitoraAtivosDiscovery = new MonitoraAtivosDiscovery();
		monitoraAtivosDiscovery.run();
		*/
		/*
		 * NetMapDTO netMapDTO = new NetMapDTO();
		 * netMapDTO.setNome("CITDFPC0470"); //netMapDTO.setNome("CITDFNTB001");
		 * //netMapDTO.setNome("10.0.1.63"); //netMapDTO.setNome("localhost");
		 * netMapDTO.setIp("10.0.1.116"); boolean agenteLocalDOTNET = false;
		 * boolean agenteServerWindows = false; if(netMapDTO.getNome() != null){
		 * agenteLocalDOTNET =
		 * monitoraAtivosDiscovery.geraInventarioAtivoAgenteLocal
		 * (netMapDTO.getNome(), dataAtualInv); }else{ agenteLocalDOTNET =
		 * monitoraAtivosDiscovery
		 * .geraInventarioAtivoAgenteLocal(netMapDTO.getIp(), dataAtualInv); }
		 * if (!agenteLocalDOTNET){ //Se nao tem AGENTE LOCAL .NET
		 * if(netMapDTO.getNome() != null){ agenteServerWindows =
		 * monitoraAtivosDiscovery
		 * .geraInventarioAtivo(IP_SERVER_WINDOWS_INVENTARIO,
		 * netMapDTO.getNome(), netMapDTO.getNome(), dataAtualInv); }else{
		 * agenteServerWindows =
		 * monitoraAtivosDiscovery.geraInventarioAtivo(IP_SERVER_WINDOWS_INVENTARIO
		 * , netMapDTO.getIp(), netMapDTO.getIp(), dataAtualInv); } }
		 */
