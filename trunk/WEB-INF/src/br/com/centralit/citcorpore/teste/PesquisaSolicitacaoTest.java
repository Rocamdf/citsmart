...
public class PesquisaSolicitacaoTest {
	public String testPesquisaSolicitacao(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			colCriterios.add(new Condition("dataFinal", "", UtilDatas.strToTimestamp(UtilDatas.dateToSTR(solicitacaoServicoDto.getDataFim()) + " 23:59:59")));
	...
	private String montaHTMLResumoSolicitacoes(ArrayList<SolicitacaoServicoDTO> resumo, StringBuffer script, HttpServletRequest request, HttpServletResponse response) throws ServiceException, Exception {
		...
		for (SolicitacaoServicoDTO r : resumo) {
			...
			if (r.getSeqReabertura() == null || r.getSeqReabertura().intValue() == 0) {
				...
				html.append("<td id='dataHoraSolicitacao'>" + UtilDatas.formatTimestamp(r.getDataHoraSolicitacao()) + "<br><br>" + UtilI18N.internacionaliza(request, "solicitacaoServico.seqreabertura") + ": <span style='color:red'><b>" + r.getSeqReabertura() + "</b></span></td>");
			}
