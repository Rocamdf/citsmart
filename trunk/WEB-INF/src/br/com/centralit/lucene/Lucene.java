...
public class Lucene {
	...
	public boolean indexarListaPalavrasGemeas(ArrayList<PalavraGemeaDTO> palGemeas) {
		try {
	...
	private boolean indexarAnexoBaseConhecimento(ArrayList<AnexoBaseConhecimentoDTO> listaAnexoBaseConhecimento) {
		try {
	...
	private boolean indexarDocumentoBaseConhecimento(BaseConhecimentoDTO baseConhecimentoDTO,boolean excluiAnexos) throws IOException {
		...
		if (baseConhecimentoDTO.ativa()){
			...
			if ((baseConhecimentoDTO.getTitulo()==null)||(baseConhecimentoDTO.getTitulo().length() <= 0))
				...
				doc.add(new Field("titulo", baseConhecimentoDTO.getTitulo(),
			...
			if (baseConhecimentoDTO.getGerenciamentoDisponibilidade() == null || baseConhecimentoDTO.getGerenciamentoDisponibilidade() != "S")
	...
	public boolean indexarBaseConhecimento(BaseConhecimentoDTO baseConhecimentoDTO,ArrayList<AnexoBaseConhecimentoDTO> listaAnexoBaseConhecimento) {
		try {
			if (baseConhecimentoDTO != null && baseConhecimentoDTO.getIdBaseConhecimento() != null) {
...
*/	
	...
	private ArrayList<String> pesquisaPalavrasGemeas(String indice) throws ParseException {
		try {
			...
			ArrayList<String> escapingSpecialChar = new ArrayList<String>();
			escapingSpecialChar.add("+");
			escapingSpecialChar.add("-");
			escapingSpecialChar.add("&&");
			escapingSpecialChar.add("||");
			escapingSpecialChar.add("!");
			...
			String[] palavras = indice.split("[ |,|;|:|.]");
			for (String palavra : palavras) {
				if (palavra != null && !palavra.trim().isEmpty() && !escapingSpecialChar.contains(palavra)) {
	...
	private String termoPesquisaLimpo(String termo){
		...
		StringBuilder newPalavra = new StringBuilder();

		String[] palavras = termo.split("[ |,|;|:|.]");
		for (String palavra : palavras) {
			if (palavra != null && !palavra.trim().isEmpty() && !escapingSpecialChar.contains(palavra)) {
				
				newPalavra = new StringBuilder();
				for (int i = 0; i < palavra.length(); i++) {
					if (newPalavra.length()<=0){
						if (!escapingSpecialChar.contains(palavra.substring(i, i+1))){
							newPalavra.append(palavra.substring(i, i+1));
						}
					} else {
						if (escapingSpecialChar.contains(palavra.substring(i, i+1))){
...
/*		if (baseConhecimentoDTO.getGerenciamentoDisponibilidade() == null || baseConhecimentoDTO.getGerenciamentoDisponibilidade().isEmpty()) {
			gerDisp = "";
		} else {
			gerDisp = baseConhecimentoDTO.getGerenciamentoDisponibilidade();
		}	
*/		
