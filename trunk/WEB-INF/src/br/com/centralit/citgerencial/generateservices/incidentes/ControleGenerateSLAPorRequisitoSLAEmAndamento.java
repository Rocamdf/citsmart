...
public class ControleGenerateSLAPorRequisitoSLAEmAndamento {
	public List execute(Integer idRequisitoSLA)
...
*/
		...
		sql = sql + "WHERE (sol.idacordonivelservico in (select idacordonivelservico from SLARequisitoSLA where idRequisitoSLA = ?)) and sol.situacao = 'EmAndamento' ";
		...
		parametersValuesBusca.add(idRequisitoSLA);
		...
		try {
			List listaDados = jdbcEngine.execSQL(sql,
					parametersValuesBusca.toArray(), 0);
			if (listaDados != null) {
				for (int i = 0; i < listaDados.size(); i++) {
					...
					if (dataHoraFim != null) {
						dataHoraLimiteStr = UtilDatas
								.formatTimestamp(dataHoraLimite);
					}
