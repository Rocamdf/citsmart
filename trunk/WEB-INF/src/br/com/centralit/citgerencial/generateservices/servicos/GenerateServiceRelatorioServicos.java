...
public class GenerateServiceRelatorioServicos extends GerencialGenerateService {
	...
	public List execute(HashMap parametersValues, Collection paramtersDefinition) throws ParseException {
		...
		if (CITCorporeUtil.SGBD_PRINCIPAL.toUpperCase().equals(SQLConfig.ORACLE)) {
			...
			String dataInicialDate = formatoBanco.format(datainicio);
			String dataFinalDate = formatoBanco.format(datafim);
				...
				try {
					...
					if (col != null && col.size() > 0) {
						...
						for (Iterator it2 = col.iterator(); it2.hasNext();) {
							...
							if (contratoDTO != null) {
								aux += "" + contratoDTO.getNumero() + "  (" + UtilDatas.dateToSTR(servicoContratoDTO.getDataInicio()) + " - "
										+ UtilStrings.nullToVazio(UtilDatas.dateToSTR(servicoContratoDTO.getDataFim())) + ") ";
