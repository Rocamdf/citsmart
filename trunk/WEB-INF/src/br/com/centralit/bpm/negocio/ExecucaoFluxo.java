...
public abstract class ExecucaoFluxo extends NegocioBpm implements IExecucaoFluxo {
	...
	private AtribuicaoFluxoDTO recuperaDelegacao(Integer idItemTrabalho, AtribuicaoFluxoDao atribuicaoTarefaDao) throws Exception {
		...
		if (colDelegacao != null && !colDelegacao.isEmpty())
			return (AtribuicaoFluxoDTO) ((List) colDelegacao).get(0);
		else
			return null;
	}
	/**
	...
	public List<TarefaFluxoDTO> recuperaTarefas(String loginUsuario, Integer idTarefa) throws Exception {
		...
		if (usuarioDto != null) {
			...
			HashMap<String, FluxoDTO> mapFluxos = new HashMap();
			HashMap<String, ElementoFluxoTarefaDTO> mapElementos = new HashMap();
			HashMap<String, PermissoesFluxoDTO> mapPermissoes = new HashMap();
			HashMap<String, UsuarioBpmDTO> mapUsuarios = new HashMap();
			HashMap<String, GrupoBpmDTO> mapGrupos = new HashMap();
			...
			for (TarefaFluxoDTO tarefaFluxoDto : tarefas) {
				...
				if (listGrupoBpmDTO != null && !tarefaFluxoDto.isSomenteAcompanhamento()) {
					// InstanciaFluxoDTO instanciaFluxoDto = new InstanciaFluxoDTO();
					// instanciaFluxoDto.setIdInstancia(tarefaFluxoDto.getIdInstancia());

					// Aqui nós recuperamos a InstanciaFluxoDTO apenas para usar o idFluxo. Podemos retornar essa informação na consulta de TarefaFluxoDTO
				...
				if (elementoFluxoDto.getVisao() != null && elementoFluxoDto.getVisao().trim() != "") {
					FormDinamicoDTO visaoDto = new FormDinamicoDao().findByIdentificador(elementoFluxoDto.getVisao().trim());
					if (visaoDto != null)
