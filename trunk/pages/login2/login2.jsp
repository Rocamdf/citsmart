...
<script type="text/javascript" src="<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/js/login2.js"></script>
...
<link type="text/css" rel="stylesheet" href="<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/novoLayout/common/include/css/login2.css"/>
		...
		<div class="wrapper">
			...
			<div class="widget">
				...
				<div class="widget-body">
					...
					<!-- <input type="hidden" name="locale" id="locale" /> -->
						<label><i18n:message key="login.login"/></label>
						<input type="text" class="input-block-level" id="user" name="user"/> 
						<label><i18n:message key="login.senha"/>
							<% UsuarioService usuarioService = (UsuarioService) ServiceLocator.getInstance().getService(UsuarioService.class, null);
											...
											if (metodoAutenticacaoProprio != null && metodoAutenticacaoProprio.trim().equalsIgnoreCase("1") ) {
												...
												if (!usuarioIsAd) {
							%>	
								<a class="password" href="#modal_alteraSenha" data-toggle="modal" id='modals-bootbox-confirm'><i18n:message key="recuperacaoSenha.esqueceuSuaSenha" /></a>
							<%
								}		
							} 
							%>	
						...
						<div class="row-fluid">
							...
							<div class="span4 center">
								<button class="btn btn-block btn-primary" onclick='validar();' type="button"><i18n:message key="login.login"/></button>
							</div>
						</div>
					</form>
					<!-- // Form END -->
					...
					<form name="form1" action="<%= CitCorporeConstantes.CAMINHO_SERVIDOR %><%= request.getContextPath() %>/pages/login/login" method="post">
							<div class="row-fluid">
								<div class="span12">
									...
									<input type="text" class="span12 margin-none" id="login_" name="login" placeholder="<i18n:message key="recuperacaoSenha.dica.loginOuEmail" />" />
								</div>
							</div>
...
<%-- 							<fieldset>
							<label for="login"><i18n:message key="recuperacaoSenha.loginOuEmail" /></label>
							<div>
								<input type="text" class="input-block-level margin-none" id="login" name="login" placeholder="<i18n:message key="recuperacaoSenha.dica.loginOuEmail" />" />
							</div>
						</fieldset> --%>
					</form>
				</div>
		...
		<p><i></i><i18n:message key="login.problema"/></p>
			<span class="glyphicons phone" data-toggle="notyfy" data-layout="topRight" data-type="primary"><i></i><b><i18n:message key="citcorpore.comum.suporte"/></b><span class=""> 0800 6363363</span></span>&nbsp; &nbsp;
			<a href="mailto:suporte.citsmart@centralit.com.br?Subject=[<i18n:message key="citcorpore.comum.suporte"/>]" target="top" data-toggle="" class="glyphicons envelope"><i></i>suporte.citsmart@centralit.com.br <span class=""></span></a>
