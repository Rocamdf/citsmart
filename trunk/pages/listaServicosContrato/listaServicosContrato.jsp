...
<div id="retPesq" style='width: 98%'>
	<button type='button' name='btnGravar' class="light"  onclick='adicionaServico();'>
		...
		<span><i18n:message key="citcorpore.comum.adicionarServicoContrato" /></span>
	</button>
	<button type='button' name='btnGravar' class="light"  onclick='adicionaVariosServico();'>
		<img src='<%=br.com.citframework.util.Constantes.getValue("CONTEXTO_APLICACAO")%>/template_new/images/icons/small/grey/plane_suitcase.png'>
		<span><i18n:message key="citcorpore.comum.adicionarVariosServicoContrato" /></span>
	</button>	
	<button type='button' name='btnGravar' class="light"  onclick='showDesempenhoContrato();'>
		<img src='<%=br.com.citframework.util.Constantes.getValue("CONTEXTO_APLICACAO")%>/template_new/images/icons/small/grey/plane_suitcase.png'>
		<span><i18n:message key="citcorpore.comum.desempenhoContrato" /></span>
	</button>	
	<button type='button' name='btnExcluir' class="light" onclick='excluirCheckedBoxes();'>
		...
		<span><i18n:message key="citcorpore.comum.removerServicosContrato" /></span>
	</button>		
	<button type='button' name='btnGerenciarSlas' class="light" onclick='gerenciarSlas();'>
		<img src="<%=br.com.citframework.util.Constantes.getValue("CONTEXTO_APLICACAO")%>/template_new/images/icons/small/grey/plane_suitcase.png">
		<span><i18n:message key="citcorpore.comum.gerenciarSlas" /></span>
	</button>
<br/>
<table cellpadding="0" cellspacing="0" width="100%" style='width: 98%' class="table table-bordered table-striped" id="tabelaServicoContrato">
	<tr style='text-align: center;' class=''>
		...
		<td>
			<b><i18n:message key="citcorpore.comum.servico"/></b>
		</td>
		<td>
			<b><i18n:message key="citcorpore.comum.situacao"/></b>
		</td>		
		<td>
			<b><i18n:message key="avaliacao.fornecedor.servico.demanda"/></b>
		</td>	
		<td>
			<b><i18n:message key="avaliacao.fornecedor.sla"/></b>
		</td>			
		<td>
			<b><i18n:message key="citcorpore.comum.datainicio"/></b>
		</td>
		<td>
			<b><i18n:message key="citcorpore.comum.datafim"/></b>
		</td>	
			<td>
			<b><input type="checkbox" title='<i18n:message key="citcorpore.comum.selecionarTodos" />' id="marcarExcluir" onclick="marcarTodosCheckbox(this);" /></b>
		</td>				
	</tr>
	<%
...
<div id="pag" style="text-align: center; display: block; width: 100%; margin-bottom: 5px;">
		...
		<label><i18n:message key="citcorpore.comum.mostrandoPagina" /></label> <%=pagAtual%> <label><i18n:message key="citcorpore.comum.mostrandoPaginaDe" /></label> <%=totalPag%> 
	<input id="btnext" class="ui-widget ui-state-default botao corner-all" type="button" onclick="paginacao(1)" value='<i18n:message key="citcorpore.comum.proximo" />' title='<i18n:message key="citcorpore.comum.proximo" />' style=" cursor: pointer" name="btnext">
		<input id="btlast" class="ui-widget ui-state-default botao corner-all" type="button" onclick="paginacao(<%=totalItens%>);" value='<i18n:message key="citcorpore.comum.ultimo" />' title='<i18n:message key="citcorpore.comum.ultimo" />' style=" cursor: pointer" name="btlast">
</div>