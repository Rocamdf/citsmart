		...
		$(this).bootstrapWizard(
		{
			onNext: function(tab, navigation, index) 
			{
				if(index==1)
				{
					...
					/*Mário Júnior 03/12/2013
					Se existir template, validar antes de passar pro próximo.*/
			...
			{
				if(!wiz.find('#idContrato').val()) { // Validação de contrato
					alert(i18n_message("solicitacaoservico.validacao.contrato"));
					wiz.find('#idContrato').focus();
					return false;
					...
					wiz.find('#idUnidade').focus();
					return false;						
				} else if(!wiz.find("#idTipoDemandaServico").val()) { // Validando os dados da solicitação
					alert(i18n_message("solicitacaoServico.tipo") + ": " + i18n_message("citcorpore.comum.campo_obrigatorio"));
					wiz.find('#idTipoDemandaServico').focus();
					return false;
				} else if(!wiz.find("#servicoBusca").val()) {
			...
			onTabShow: function(tab, navigation, index) 
			{
				...
				/*Elevando o scrool ao topo da página*/
				$('html, body').animate({ 
					scrollTop: 0
				}, 500);
			},
