...
<script>
	...
	function excluirImagemUpload(path){
		if (confirm(i18n_message("citcorpore.comum.confirme.desejaexcluiranexo"))){
			window.location = '<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/pages/uploadExcluirRequisicaoMudanca/uploadExcluirRequisicaoMudanca.load?path=' + path;
		}
	}
	...
	function excluirAnexo(id, nomeArquivo){
		if(confirm(i18n_message("citcorpore.comum.confirme.exclusaoanexo"))){
			window.location = '<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/pages/uploadExcluirRequisicaoMudanca/excluirAnexo.do?nameFile=' + nomeArquivo + '&id='+id;	
		}		
	}
	<%
