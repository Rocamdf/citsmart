...
ALTER TABLE gruporequisicaomudanca ADD nomegrupo VARCHAR(250);
...
alter table historicoic add idcontrato integer null;
alter table historicoic add idliberacao integer null;
alter table historicoic add idresponsavel integer null;
alter table historicoic add ativofixo varchar(255) null;
...
alter table solicitacaoservico add vencendo varchar(1) NULL;
...
CREATE  TABLE regraescalonamento (
  idregraescalonamento int NOT NULL ,
  idtipogerenciamento int NOT NULL ,
  ...
  intervalonotificacao int NOT NULL ,
  enviaremail VARCHAR(1),
  criaproblema VARCHAR(1),
  prazocriarproblema int NULL ,
...
CREATE  TABLE escalonamento (
  idescalonamento int NOT NULL ,
  idregraescalonamento int NOT NULL ,
  idgrupoexecutor int NOT NULL ,
  prazoexecucao int NOT NULL ,
  datainicio date null,
  datafim date null,
...
alter table regraescalonamento add tipodataescalonamento int;
...
alter table escalonamento add idprioridade int null;

-- Fim Euler 10/09/2013

-- Início Bruno 10/09/2013

alter table solicitacaoservico add vencendo varchar(1) NULL;
alter table solicitacaoservico add criouproblemaautomatico varchar(1) NULL;
...
alter table itemrequisicaoproduto alter column situacao varchar(50) null;

-- Fim Cleison

-- Inicio Mário Haysaki Júnior 12/09/2013

ALTER TABLE projetos add deleted varchar(1) DEFAULT NULL;
...
alter table requisicaomudanca add vencendo varchar(1) NULL;
...
alter table itemconfiguracao ADD  datahoradesinstalacao smalldatetime;
...
(
   idproblema int, 
   idescalonamento int, 
   PRIMARY KEY (idproblema, idescalonamento), 
...
alter table problema add vencendo varchar(1) NULL;

-- Fim - Bruno Franco

-- Inicio - Riubbe Da Silva Oliveira 16/09/2013

alter table tipomovimfinanceiraviagem add constraint pk_tipomovimfinanceiraviagem primary key(idtipoMovimFinanceiraViagem);

create table controlefinanceiroviagem (
    idcontrolefinanceiroviagem bigint not null,
    idresponsavel integer default null,
    idmoeda integer default null,
...
create table itemcontrolefinanceiroviagem (
	...
	dataexecucao smalldatetime default null,
	datahoraprazocotacao smalldatetime default null
);
...
CREATE TABLE prestacaocontasviagem (
  ...
  idresponsavel integer DEFAULT NULL,
  idaprovacao integer DEFAULT NULL,
  idsolicitacaoservico bigint DEFAULT NULL,
  idempregado integer DEFAULT NULL,
  datahora timestamp NULL,
...
alter table controlefinanceiroviagem add idresponsavelcompras integer null;

-- Fim - Riubbe Da Silva Oliveira

-- Inicio - Bruno Franco 18/09/2013

ALTER TABLE regraescalonamento ALTER COLUMN tempoexecucao INT NULL;
...
ALTER TABLE problema ALTER COLUMN descricao  varchar(3000);

-- Fim - Bruno Franco

-- Inicio  Maycon 25/09/2013

alter table solicitacaoservico add idusuarioresponsavelatual integer;

-- Fim Maycon

-- Inicio - Bruno Franco 04/10/2013

ALTER TABLE baseconhecimento ADD idsolicitacaoservico integer null;

-- Fim Bruno Franco

-- Inicio - Maycon 09/10/2013

ALTER TABLE parametrocorpore add  tipodado varchar(50);

-- Fim Maycon

-- Inicio - Rodrigo Pecci Acorse 17/10/2013

CREATE UNIQUE INDEX INDEX_EMP_IDEMPREGADO ON empregados (idempregado ASC);
CREATE INDEX INDEX_GRUPO ON gruposempregados (idgrupo ASC);
CREATE INDEX INDEX_GRUPO ON contratosgrupos (idgrupo ASC);
CREATE INDEX INDEX_CONTRATO ON contratosgrupos (idcontrato ASC);
...
CREATE INDEX idx_idatribuicao ON bpm_atribuicaofluxo (idatribuicao ASC);
CREATE INDEX idx_idelemento ON bpm_elementofluxo (idelemento ASC);
CREATE INDEX idx_idfluxo ON bpm_fluxo (idfluxo ASC);
CREATE INDEX idx_idhistoricoitemtrabalho ON bpm_historicoitemtrabalho (idhistoricoitemtrabalho ASC);
CREATE INDEX idx_idinstancia ON bpm_instanciafluxo (idinstancia ASC);
CREATE INDEX idx_iditemtrabalho ON bpm_itemtrabalhofluxo (iditemtrabalho ASC);
CREATE INDEX idx_idobjetoinstancia ON bpm_objetoinstanciafluxo (idobjetoinstancia ASC);
CREATE INDEX idx_idelementoorigem ON bpm_sequenciafluxo (idelementoorigem ASC);
CREATE INDEX idx_idtipofluxo ON bpm_tipofluxo (idtipofluxo ASC);
...
ALTER TABLE historicoitemrequisicao ADD acao VARCHAR(100);
...
ALTER TABLE rh_entrevistacandidato ADD metodosadicionais TEXT;

ALTER TABLE rh_entrevistacandidato ADD notaavaliacao DECIMAL(6,2);

ALTER TABLE rh_requisicaopessoal ADD iduf INTEGER;

ALTER TABLE rh_requisicaopessoal ADD tipocontratacao CHAR(11);

ALTER TABLE rh_requisicaopessoal ADD idpais INTEGER;

ALTER TABLE rh_requisicaopessoal ADD qtdcandidatosaprovados INTEGER;

ALTER TABLE rh_entrevistacandidato ADD adimitido BIT DEFAULT 0;
