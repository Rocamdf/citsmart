...
function TCPopup(field, palette, pickerFile) {
	this.field = field;
	this.initPalette = !palette || palette > 3 ? 0 : palette;
	var w = 194, h = 240,
	move = screen ? 
		',left=' + ((screen.width - w) >> 1) + ',top=' + ((screen.height - h) >> 1) : '', 
	o_colWindow = window.open(pickerFile, null, "help=no,status=no,scrollbars=no,resizable=no" + move + ",width=" + w + ",height=" + h + ",dependent=yes", true);
...
function TCSelect(c) {
	var hexColor = '#' + c.toUpperCase();
	this.field.style.backgroundColor = hexColor;
	this.field.style.fontColor = hexColor;
	this.field.value = HexToDec(c.toUpperCase());
	this.win.close();
}
