...
$(function()
{
	...
	/* wysihtml5 */
	/*
	 * Rodrigo Pecci Acorse - 13/11/2013 - #123390
	 * Alteração para limitar a quantidade de caracteres nos textareas que utilizam o editor wysihtml5
	 * 
	 */
	if ($('textarea.wysihtml5').size() > 0) {
		$('textarea.wysihtml5').wysihtml5({
			"events": {
				"change": function() { 
					var limit = 65000;
					var value = $(this)[0].composer.iframe.contentDocument.body.innerHTML;
	                var chars = value.length;

	                if (chars > limit) {
	                	alert('O texto inserido ultrapassou o limite de caracteres permitidos.');
	                	$(this)[0].composer.iframe.contentDocument.body.innerHTML = value.substr(0, limit);
	                }
				}
			}
		});
