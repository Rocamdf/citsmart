...
public class ItemConfiguracaoDao extends CrudDaoDefaultImpl {
	...
	private static final long serialVersionUID = 7648160906937369094L;
	...
	public Collection findByIdItemConfiguracaoPai(Integer parm) throws Exception {
		...
		StringBuffer sqlItemConfiguracao = new StringBuffer();

		sqlItemConfiguracao.append("SELECT itemConfiguracao.identificacao, itemConfiguracao.idItemConfiguracao, itemConfiguracao.status, tipoitemconfiguracao.imagem ");
		...
		sqlItemConfiguracao.append("WHERE itemConfiguracao.idItemConfiguracaoPai = ? ");
	...
	public Collection findByIdItemConfiguracaoPai(ItemConfiguracaoDTO itemConfiguracaoDTO) throws Exception {
		...
		condicao.add(new Condition("identificacao", "=", itemConfiguracaoDTO.getIdentificacao()));

		if (itemConfiguracaoDTO.getIdItemConfiguracaoPai() != null) {
			condicao.add(new Condition("idItemConfiguracaoPai", "=", itemConfiguracaoDTO.getIdItemConfiguracaoPai()));
			condicao.add(new Condition("idTipoItemConfiguracao", "=", itemConfiguracaoDTO.getIdTipoItemConfiguracao()));
		}
	...
	public void deleteByIdItemConfiguracaoPai(Integer parm) throws Exception {
		List condicao = new ArrayList();
		condicao.add(new Condition("idItemConfiguracaoPai", "=", parm));
		super.deleteByCondition(condicao);
	}

	public Collection findByIdTipoItemConfiguracao(String parm) throws Exception {
		List condicao = new ArrayList();
		List ordenacao = new ArrayList();
	...
	public void deleteByIdTipoItemConfiguracao(Integer parm) throws Exception {
		List condicao = new ArrayList();
		condicao.add(new Condition("idTipoItemConfiguracao", "=", parm));
		super.deleteByCondition(condicao);
	}
	...
	public Collection<ItemConfiguracaoDTO> listByIdentificacao(PesquisaItemConfiguracaoDTO pesquisa) throws Exception {
		List parametro = new ArrayList();
		...
		if (pesquisa.getInstalacao() != null) {
			sqlItemConfiguracao.append("AND itemConfiguracaoEvento.tipoexecucao = ? ");
			parametro.add(pesquisa.getInstalacao());
		}

		if (pesquisa.getDesinstalacao() != null) {
			sqlItemConfiguracao.append("AND itemConfiguracaoEvento.tipoexecucao = ? ");
	...
	public Collection<ItemConfiguracaoDTO> pesquisaDataExpiracao(java.util.Date data) throws Exception {
		List parametro = new ArrayList();
	...
	public Collection<ItemConfiguracaoDTO> listItemConfiguracaoFilho(ItemConfiguracaoDTO itemConfiguracao) throws Exception {
		List parametro = new ArrayList();

		StringBuffer sb = new StringBuffer();

		sb.append("SELECT idItemConfiguracao ");
	...
	public Collection<ItemConfiguracaoDTO> listAtivos() throws Exception {
		String sql = "SELECT itemConfiguracao.identificacao, itemConfiguracao.idItemConfiguracao, itemConfiguracao.status, tipoitemconfiguracao.imagem, itemConfiguracao.dtultimacaptura FROM ITEMCONFIGURACAO itemConfiguracao " +
" left join tipoitemconfiguracao on itemConfiguracao.idtipoitemconfiguracao = tipoitemconfiguracao.idtipoitemconfiguracao " + 
" WHERE itemConfiguracao.IDITEMCONFIGURACAOPAI IS NULL " + 
" AND itemConfiguracao.DATAFIM " + 
" IS NULL ORDER BY length(itemConfiguracao.identificacao), itemConfiguracao.identificacao";
	...
	public Collection<ItemConfiguracaoDTO> listItensSemGrupo(String criticidade, String status, String sistemaOperacional, String grupoTrabalho, String tipoMembroDominio, String usuario, String processador, List softwares) throws Exception {
		...
		if(sistemaOperacional != null && !sistemaOperacional.equals("")){
			...
			sqlItemConfiguracao.append("(SELECT iditemconfiguracao FROM valor INNER JOIN caracteristica ON valor.idcaracteristica = caracteristica.idcaracteristica  where datafim is null AND tagcaracteristica = 'OSNAME' AND valorstr like '" + itemStr + "%') ");
			sqlItemConfiguracao.append(") ");
		}
		if(grupoTrabalho != null && !grupoTrabalho.equals("")){
			...
			sqlItemConfiguracao.append("WHERE itemConfiguracaoAux.DATAFIM IS NULL ");
			sqlItemConfiguracao.append("AND itemConfiguracaoAux.iditemconfiguracao IN "); 
			sqlItemConfiguracao.append("(SELECT iditemconfiguracao FROM valor INNER JOIN caracteristica ON valor.idcaracteristica = caracteristica.idcaracteristica  where datafim is null AND tagcaracteristica = 'WORKGROUP' AND valorstr like '" + itemStr + "%') ");
			sqlItemConfiguracao.append(") ");
		}
		if(tipoMembroDominio != null && !tipoMembroDominio.equals("")){
			...
			sqlItemConfiguracao.append("WHERE itemConfiguracaoAux.DATAFIM IS NULL ");
			sqlItemConfiguracao.append("AND itemConfiguracaoAux.iditemconfiguracao IN "); 
			sqlItemConfiguracao.append("(SELECT iditemconfiguracao FROM valor INNER JOIN caracteristica ON valor.idcaracteristica = caracteristica.idcaracteristica  where datafim is null AND tagcaracteristica = 'DESCDOMAINROLE' AND valorstr like '" + itemStr + "%') ");
			sqlItemConfiguracao.append(") ");
		}
		if(usuario != null && !usuario.equals("")){
			...
			sqlItemConfiguracao.append("WHERE itemConfiguracaoAux.DATAFIM IS NULL ");
			sqlItemConfiguracao.append("AND itemConfiguracaoAux.iditemconfiguracao IN "); 
			sqlItemConfiguracao.append("(SELECT iditemconfiguracao FROM valor INNER JOIN caracteristica ON valor.idcaracteristica = caracteristica.idcaracteristica  where datafim is null AND tagcaracteristica = 'USERID' AND valorstr like '%" + itemStr.trim() + "%') ");
			sqlItemConfiguracao.append(") ");
		}	
		if(processador != null && !processador.equals("")){
			...
			sqlItemConfiguracao.append("WHERE itemConfiguracaoAux.DATAFIM IS NULL ");
			sqlItemConfiguracao.append("AND itemConfiguracaoAux.iditemconfiguracao IN "); 
			sqlItemConfiguracao.append("(SELECT iditemconfiguracao FROM valor INNER JOIN caracteristica ON valor.idcaracteristica = caracteristica.idcaracteristica  where datafim is null AND tagcaracteristica = 'PROCESSORT' AND valorstr like '%" + itemStr.trim() + "%') ");
			sqlItemConfiguracao.append(") ");
		}	
		if (softwares != null && softwares.size() > 0){
			...
			for (Iterator it = softwares.iterator(); it.hasNext();){
				...
				if (soft != null && !soft.trim().equalsIgnoreCase("")){
					...
					sqlItemConfiguracao.append("WHERE itemConfiguracaoAux.DATAFIM IS NULL ");
					sqlItemConfiguracao.append("AND itemConfiguracaoAux.iditemconfiguracao IN "); 
					sqlItemConfiguracao.append("(SELECT iditemconfiguracao FROM valor INNER JOIN caracteristica ON valor.idcaracteristica = caracteristica.idcaracteristica  where datafim is null AND tagcaracteristica = 'PUBLISHER' AND " + uppFieldFunction + " like '%" + itemStr.trim().toUpperCase() + "%') ");
					sqlItemConfiguracao.append(") ");					
				}
			}
		}
	...
	public void atualizaGrupo(ItemConfiguracaoDTO itemConfiguracaoDTO) throws Exception {
		List parametros = new ArrayList();
		StringBuffer sqlItemConfiguracao = new StringBuffer();
	...
	public Collection<ItemConfiguracaoDTO> listByEvento(Integer idEvento) throws Exception {
		String sql = "SELECT ic.iditemconfiguracao, ic.identificacao FROM " + getTableName() + " AS ic INNER JOIN eventoitemconfiguracao AS eic ON eic.iditemconfiguracao = ic.iditemconfiguracao WHERE eic.idevento = ?";
	...
	public Integer quantidadeMidiaSoftware(ItemConfiguracaoDTO itemDTO) throws Exception {
		...
		sql.append("select *from " + getTableName() + 
				" itemConfiguracao inner join midiasoftware midiaSoftware " +
	...
	public boolean VerificaSeCadastrado(ItemConfiguracaoDTO itemDTO) throws Exception {
		...
		List list = new ArrayList();
		StringBuffer sql = new StringBuffer();
		sql.append("select idItemConfiguracao from " + getTableName() + " where identificacao = ? ");
		parametro.add(itemDTO.getIdentificacao());
		if (itemDTO.getIdGrupoItemConfiguracao() != null) {
			sql.append("and idGrupoItemConfiguracao = ? ");
			parametro.add(itemDTO.getIdGrupoItemConfiguracao());
		}
	...
	// bruno.aquino
	public Collection listItemConfiguracaoPorSolicitacaoServicoEmAndamento(int idSolicitante, String situacao, String idservico) throws Exception {
	...
	public Collection<ItemConfiguracaoDTO> listItemConfiguracaoByIdProblema(Integer problema) throws Exception {
		String sql = "SELECT ic.identificacao, ic.familia, ic.classe, ic.localidade FROM " + getTableName() + " AS ic INNER JOIN problemaitemconfiguracao AS eic ON eic.iditemconfiguracao = ic.iditemconfiguracao "
	...
	public Collection<ItemConfiguracaoDTO> listItemConfiguracaoByIdIncidente(Integer idIncidente) throws Exception {
		String sql = "SELECT ic.identificacao, ic.familia, ic.classe, ic.localidade FROM " + getTableName() + " AS ic INNER JOIN itemcfgsolicitacaoserv AS eic ON eic.iditemconfiguracao = ic.iditemconfiguracao "
	 ...
	 */
	public List<ItemConfiguracaoDTO> findByConhecimento(BaseConhecimentoDTO baseConhecimentoDto) throws Exception {
		List parametro = new ArrayList();
		List listRetorno = new ArrayList();
		List list = new ArrayList();
		StringBuffer sql = new StringBuffer();
	...
	public List<ItemConfiguracaoDTO> listItemConfiguracaoFinalizadosByIdItemconfiguracao(ItemConfiguracaoDTO itemConfiguracaoDto) throws Exception{
		List parametro = new ArrayList();
	...
	public ItemConfiguracaoDTO findByIdentificacaoItemConfiguracao(ItemConfiguracaoDTO itemConfiguracaoDTO) throws Exception {
		...
		sqlItemConfiguracao.append("AND identificacao like '%"+itemConfiguracaoDTO.getIdentificacao()+"%' ");
	...
	public Collection<ItemConfiguracaoDTO> listByIdItemPaiAndTagTipoItemCfg(Integer idItemConfiguracaoPai, String tagTipoCfg) throws Exception {
		String sql = "SELECT " + this.getNamesFieldsStr() + " " +
				"FROM itemconfiguracao where iditemconfiguracaopai = ? AND idtipoitemconfiguracao IN " +
				"(SELECT idtipoitemconfiguracao FROM tipoitemconfiguracao where tagtipoitemconfiguracao = ?) " +
				"AND datafim IS NULL";
