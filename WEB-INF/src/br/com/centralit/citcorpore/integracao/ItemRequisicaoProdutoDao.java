...
public class ItemRequisicaoProdutoDao extends CrudDaoDefaultImpl {
    ...
    public Collection findTodosByIdSolicitacaoServico(Integer parm) throws Exception {
        List condicao = new ArrayList();
        List ordenacao = new ArrayList(); 
        condicao.add(new Condition("idSolicitacaoServico", "=", parm)); 
        ordenacao.add(new Order("idItemRequisicaoProduto"));
    ...
    public Collection findByIdItemCotacao(Integer idItemCotacao) throws Exception {
        List condicao = new ArrayList();
        List ordenacao = new ArrayList(); 
    ...
    public Collection findByIdItemCotacaoOrderQtde(Integer idItemCotacao) throws Exception {
        List condicao = new ArrayList();
        List ordenacao = new ArrayList(); 
        condicao.add(new Condition("idItemCotacao", "=", idItemCotacao)); 
    ...
    public Collection<ItemRequisicaoProdutoDTO> recuperaItensParaCotacao(Date dataInicio, Date dataFim, Integer idCentroCusto,
        ...
        if (dataInicio != null) {
            ...
            parametro.add(Timestamp.valueOf(UtilDatas.dateToSTR(dataInicio, "yyyy-MM-dd") + " 00:00:00"));
        }
        if (dataFim != null) {
            sql.append("    AND sol.dataHoraSolicitacao <= ? ");
            parametro.add(Timestamp.valueOf(UtilDatas.dateToSTR(dataFim, "yyyy-MM-dd") + " 23:59:59"));
        }
        if (idProjeto != null) {
