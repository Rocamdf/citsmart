...
public class OSDao extends CrudDaoDefaultImpl {
	...
	private static final long serialVersionUID = 1871577016257124637L;
	...
	public Collection findByIdOsPai(Integer idospai) throws Exception {
		List condicao = new ArrayList();
		List ordenacao = new ArrayList();
	...
	public void updateSituacao(Integer idOs, Integer situacao) throws Exception {
	...
	public void updateObsFinal(Integer idOs, String obsFinal) throws Exception {
		OSDTO osDTO = new OSDTO();
		osDTO.setIdOS(idOs);
	...
	public void updateQuantidade(Integer idOs, Integer quantidade) throws Exception {
		OSDTO osDTO = new OSDTO();
		osDTO.setIdOS(idOs);
	...
	public void cancelaOSFilhas(OSDTO os) throws Exception{
		Collection<OSDTO> resp = retornaSeExisteOSFilha(os.getIdOS());
		if(resp != null && resp.size() > 0){
			for (OSDTO osDto : resp) {
				updateSituacao(osDto.getIdOS(), OSDTO.CANCELADA);
			}
		}
	}
	 ...
	 * @throws Exception
	 */
	public Collection retornaSeExisteOSFilha(Integer idOSPai) throws Exception{
		List lstParametros = new ArrayList();
		String sql = "SELECT * FROM "+ getTableName() +" WHERE idospai = ? ";
		...
		if (situacao != null || (dataInicio != null || dataFim != null)) {
			...
			if (situacao != null) {
				...
				for (int i = 0; i < situacao.length; i++) {
					...
					strSqlAux = strSqlAux + "" + situacao[i].intValue() + "";
				}
				sql = sql + strSqlAux;
				sql = sql + ")) ";
			}
	...
	public Collection listOSByIdAtividadePeriodica(Integer idatividadeperiodica) throws Exception {
	 ...
	 */
	public List<OSDTO> buscaOsEmCriacao() throws Exception {

		List parametros = new ArrayList();
		parametros.add(OSDTO.EM_CRIACAO.intValue());

		// Selecionar o id das OS que estão criação
		String sql = "SELECT idos FROM " + getTableName() + " WHERE situacaoos = ? ";
	...
	public Collection<RelatorioOrdemServicoUstDTO> listaAnos() throws Exception {
		...
		if(CITCorporeUtil.SGBD_PRINCIPAL.trim().equalsIgnoreCase(SQLConfig.ORACLE) || CITCorporeUtil.SGBD_PRINCIPAL.toUpperCase().equals(SQLConfig.POSTGRESQL) ) {
			sql.append("select distinct  EXTRACT(YEAR FROM os.datainicio) from " + getTableName() + " order by EXTRACT(YEAR FROM os.datainicio)  ");
		} else{
			
			sql.append("select distinct  year(os.datainicio) from " + getTableName() + " order by year(os.datainicio)  ");
