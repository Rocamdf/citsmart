		...
		try {
			...
			if (CAMINHO_RELATIVO_PAGINA_LOGIN == null || CAMINHO_RELATIVO_PAGINA_LOGIN.trim().equalsIgnoreCase("")) {
				...
				if (parametroCITSmart.trim().equalsIgnoreCase("N") || parametroCITSmart.trim().equalsIgnoreCase("")) {
					...
					CAMINHO_RELATIVO_PAGINA_LOGIN = Constantes.getValue("SERVER_ADDRESS") + Constantes.getValue("CONTEXTO_APLICACAO") + "/pages/portal/portal.load";
				}
			}
...
/*		if (requestedPath.endsWith("origemAtendimento/origemAtendimento.load")) {
		...
		if (requestedPath.endsWith("pages/templateCurriculo/templateCurriculo.load")) {
			return true;
		}
