 ...
 * 
 */
@SuppressWarnings({"unused","finally","unchecked"})
public class MenuCITSmart extends BodyTagSupport {
	...
	public String renderizacaoVertical() throws Exception {
		...
		for (MenuDTO submenu : menus) {
			String iconMenu = "";
			html.append("<a href=\"javascript:void(0)\" id='itemMM" + submenu.getIdMenu() + "' style='background:url(" + CAMINHO_IMAGENS + submenu.getImagem() + ") no-repeat;' class=\"easyui-menubutton\" data-options=\"menu:'#mm" + submenu.getIdMenu() + "'" + iconMenu + "\">" +  submenu.getNome() + "</a>\n");
		}
		html.append("</div>");
		...
		try {
			for (MenuDTO submenu : listaDeMenus) {
				...
				if (novaListaSubMenus != null && !novaListaSubMenus.isEmpty()) {
					...
					if (indice > 0) {
						sb.append("		<div id=\"mmSUB" + submenu.getIdMenu() + "\" data-options=\"" + iconMenu + "\" onclick=\"chamaItemMenu('" + link + "')\">" + submenu.getNome() + "</div>\n");
					}
				}
			}
		} catch (Exception e) {
	...
	public String validarPermissaoDeBotao(HttpServletRequest request) throws Exception {
	    if ((usuario != null && usuario.getLogin() != null) && (usuario != null && usuario.getLogin().equalsIgnoreCase("admin") || usuario.getLogin().equalsIgnoreCase("consultor"))){ //Permissao total
	    	return "";
	    }
		StringBuffer html = new StringBuffer();
