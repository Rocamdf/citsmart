...
public class AgendarAtividade2 extends AjaxFormAction {
			...
			for (Iterator it = colAgendamentos.iterator(); it.hasNext();) {
				...
				if (colProgs != null) {
					for (Iterator itProg = colProgs.iterator(); itProg.hasNext();) {
						...
						stringBuilder.append("</td>");
						stringBuilder.append("<td style='border:1px solid black'>");
						stringBuilder.append(UtilI18N.internacionaliza(request, "citcorpore.comum.data") + ": " + UtilDatas.dateToSTR(atividadePeriodicaAux.getDataCriacao()));
						...
						stringBuilder.append("</td>");
						stringBuilder.append("<td style='border:1px solid black'>");
						stringBuilder.append("<b><u>" + UtilDatas.dateToSTR(programacaoAtividadeDTO.getDataInicio()) + " " + programacaoAtividadeDTO.getHoraInicio() + "</u></b>");
						...
						if (colExecucoes != null) {
							for (Iterator itExc = colExecucoes.iterator(); itExc.hasNext();) {
								...
								strExecucao.append(UtilI18N.internacionaliza(request, "gerenciaservico.agendaratividade.dataexecucao") + " "
										+ UtilDatas.dateToSTR(execucaoAtividadePeriodicaDTO.getDataExecucao()) + " " + execucaoAtividadePeriodicaDTO.getHoraExecucao());
								strExecucao.append("<br>" + UtilI18N.internacionaliza(request, "gerenciaservico.agendaratividade.dataregistro") + " "
										+ UtilDatas.dateToSTR(execucaoAtividadePeriodicaDTO.getDataRegistro()) + " " + execucaoAtividadePeriodicaDTO.getHoraRegistro());
	...
	public void save(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ocorr.append(UtilI18N.internacionaliza(request, "gerenciaservico.agendaratividade.dataagendamento") + " " + UtilDatas.dateToSTR(atividadePeriodicaDTO.getDataInicio()));
