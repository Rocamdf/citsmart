...
public class RelatorioQuantitativoBaseConhecimento extends AjaxFormAction {
	...
	public void imprimirRelatorioQuantitativoListaBaseConhecimento(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Integer totalConhecimentoVinculado = 0;
		Integer totalConhecimentoNaoVinculado = 0;
		
		Collection<RelatorioQuantitativoBaseConhecimentoDTO> listaDadosRelatorio = new ArrayList<RelatorioQuantitativoBaseConhecimentoDTO>();
		Collection<BaseConhecimentoDTO> listaQuantitativoListaBaseConhecimento =  getBaseConhecimentoService().consultaConhecimentoQuantitativoEmLista(baseConhecimentoDTO);
		
		RelatorioQuantitativoBaseConhecimentoDTO relatorioQuantitativoBaseConhecimentoDTO = new RelatorioQuantitativoBaseConhecimentoDTO();
		
		if (listaQuantitativoListaBaseConhecimento != null) {
			for (BaseConhecimentoDTO baseConhecimentoDTO2 : listaQuantitativoListaBaseConhecimento) {
				if(baseConhecimentoDTO2 != null){
					Collection<BaseConhecimentoDTO> listaTemp = new ArrayList<BaseConhecimentoDTO>();
					listaTemp.add(baseConhecimentoDTO2);
					relatorioQuantitativoBaseConhecimentoDTO.setListaConhecimentoQuantitativoEmLista(listaTemp);
					listaDadosRelatorio.add(relatorioQuantitativoBaseConhecimentoDTO);
				}
			}
		}
		...
		String strCompl = "" + dt.getTime();
		String caminhoJasper = CITCorporeUtil.caminho_real_app + Constantes.getValue("CAMINHO_RELATORIOS") + "RelatorioQuantitativoBaseConhecimentoVincScript.jasper";
		String diretorioReceita = CITCorporeUtil.caminho_real_app + "/tempFiles";
		String diretorioRelativoOS = Constantes.getValue("SERVER_ADDRESS") + Constantes.getValue("CONTEXTO_APLICACAO") + "/tempFiles";
		...
		parametros.put("dataInicio", baseConhecimentoDTO.getDataInicio() );
		parametros.put("dataFim", baseConhecimentoDTO.getDataFim() );
		parametros.put("CONHECIMENTO_VINCULADO", totalConhecimentoVinculado);
		parametros.put("CONHECIMENTO_NAO_VINCULADO", totalConhecimentoNaoVinculado);
		parametros.put("Logo", LogoRel.getFile());
		
		if (listaQuantitativoListaBaseConhecimento != null && listaQuantitativoListaBaseConhecimento.size() == 0) {
			document.alert(UtilI18N.internacionaliza(request, "citcorpore.comum.relatorioVazio") );
			document.getJanelaPopupById("JANELA_AGUARDE_MENU").hide();
			return;
		}
		...
		try{
			...
			JasperExportManager.exportReportToPdfFile(print, diretorioReceita + "/RelatorioQuantitativoBaseConhecimentoVincScript" + strCompl + "_" + getUsuario().getIdUsuario() + ".pdf");
			
			document.executeScript("window.open('" + Constantes.getValue("SERVER_ADDRESS") + Constantes.getValue("CONTEXTO_APLICACAO") + "/printPDF/printPDF.jsp?url="
					+ diretorioRelativoOS + "/RelatorioQuantitativoBaseConhecimentoVincScript" + strCompl + "_" + getUsuario().getIdUsuario() + ".pdf')");
		} catch(OutOfMemoryError e) {
			document.alert(UtilI18N.internacionaliza(request, "citcorpore.erro.erroServidor"));
		}
	...
	public void imprimirRelatorioQuantitativoListaBaseConhecimentoXls(DocumentHTML document, HttpServletRequest request, HttpServletResponse response) throws Exception {
