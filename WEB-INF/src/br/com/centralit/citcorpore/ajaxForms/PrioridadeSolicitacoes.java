...
public class PrioridadeSolicitacoes extends AjaxFormAction {
	...
	private void carregaValoresImpacto(DocumentHTML document, HttpServletRequest request) throws ServiceException, Exception{
		...
		String impactoSerelializado = br.com.citframework.util.WebUtil.serializeObjects(lista);
	...
	private void carregaValoresUrgencia(DocumentHTML document, HttpServletRequest request) throws ServiceException, Exception{
		...
		String urgenciaSerelializado = br.com.citframework.util.WebUtil.serializeObjects(lista);
...
//			}
	...
	private Collection<ImpactoDTO> verificaImpactoExiste(HttpServletRequest request, Collection<ImpactoDTO> listaImpacto) throws ServiceException, Exception{
		Collection<ImpactoDTO> listAux = new ArrayList<ImpactoDTO>();
		
		if(listaImpacto != null){
			for(ImpactoDTO imp : listaImpacto){
				if(!getPrioridadeSolicitacoesService(request).verificaImpactoJaExiste(imp)){
					listAux.add(imp);
				}
			}
		}
	...
	private Collection<UrgenciaDTO> verificaUrgenciaExiste(HttpServletRequest request, Collection<UrgenciaDTO> listaUrgencia) throws ServiceException, Exception{
		Collection<UrgenciaDTO> listAux = new ArrayList<UrgenciaDTO>();
		
		if(listaUrgencia != null){
			for(UrgenciaDTO urgencia : listaUrgencia){
				if(!getPrioridadeSolicitacoesService(request).verificaUrgenciaJaExiste(urgencia)){
					listAux.add(urgencia);
				}
			}
		}
		
		return listAux = listAux.size()==0?null:listAux;
	}
