...
public class AnexoMudanca extends AjaxFormAction {
    ...
    protected void restaurarAnexos(HttpServletRequest request) throws ServiceException, Exception {
		...
		Collection<AnexoMudancaDTO> anexos = this.getBarraFerramentasService().consultarAnexosMudanca(1);
	
		Collection colUploadsGED = new ArrayList();
		String nomeDoArquivo = null;
		if (anexos != null && !anexos.isEmpty()) {
		    for (AnexoMudancaDTO anexo : anexos) {
    ...
    public AnexoMudancaService getBarraFerramentasService() throws ServiceException, Exception {
    	return (AnexoMudancaService) ServiceLocator.getInstance().getService(AnexoMudanca.class, null);
    }
}
