	...
	public String geraAvaliacaoNagiosNDOUtils(DocumentHTML document, HttpServletRequest request,
		...
		if (recursoDTO.getIdCalendario() != null){
			...
			while(d.compareTo(dataFim) <= 0){
				System.out.println("Verificando---> " + UtilDatas.dateToSTR(d));
			...
			for (Iterator itLog = colLogs.iterator(); itLog.hasNext();){
				...
				if ("UP".equalsIgnoreCase(log.getStatus().trim())){
					...
					if (horaQueda != null){
						...
						table += "</td>";
						table += "<td style='border:1px solid black'>";
							table += "" + UtilDatas.dateToSTR(horaQueda) + " " + UtilDatas.formatHoraFormatadaHHMMSSStr(horaQueda) + "";
						table += "</td>";
						table += "<td style='border:1px solid black'>";
							table += "" + UtilDatas.dateToSTR(horaRetorno) + " " +  UtilDatas.formatHoraFormatadaHHMMSSStr(horaRetorno) + "";
						table += "</td>";								
						table += "</tr>";
		...
		if (colBlackoutsTotais.size() > 0){
			...
			for (Iterator it = colBlackoutsTotais.iterator(); it.hasNext();){
				...
				if (programacaoAtividadeDTO != null){
					table += UtilDatas.dateToSTR(execucaoAtividadePeriodicaDTO.getDataExecucao()) + " - " + execucaoAtividadePeriodicaDTO.getHoraExecucao() + " - Tempo: " + programacaoAtividadeDTO.getDuracaoEstimada() + " min.<br>";
