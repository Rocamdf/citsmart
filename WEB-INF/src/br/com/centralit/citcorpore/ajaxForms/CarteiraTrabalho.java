   ...
   /*
	...
	public void listTimeSheet(DocumentHTML document, HttpServletRequest request,
		...
		if (colTimes != null){
			for (Iterator it = colTimes.iterator(); it.hasNext();){
				...
				strTables += "<tr>" +
						"<td>" + strLinhaBase  + "</td>" +
						"<td>" + UtilDatas.dateToSTR(timeSheetProjetoAux.getData()) + "</td>" +
						...
						"<td>" + UtilDatas.formatTimestamp(timeSheetProjetoAux.getDataHoraReg()) + "</td>" +
	...
	public void filtrarGantt(DocumentHTML document, HttpServletRequest request,
		...
		if (colCarteira != null && colCarteira.size() > 0){
			strBuff = gerarGantt(colCarteira, document);
		}
	 ...
	 */
	private StringBuffer gerarGantt(
			Collection<TarefaLinhaBaseProjetoDTO> lista, DocumentHTML document) throws ServiceException, Exception {
		...
		StringBuffer atvAtrasadas = new StringBuffer();
		boolean existeAtrasada = false;
		atvAtrasadas.append("<table width='100%'>");
		atvAtrasadas.append("<tr><td colspan='20'><b><i18n:message key='carteiraTrabalho.listaTarefaAtrasada' /></b></td></tr>");
		atvAtrasadas.append("<tr><td>&nbsp;</td><td><b><i18n:message key='carteiraTrabalho.projeto' /></b></td><td><b><i18n:message key='carteiraTrabalho.tarefa' /></b></td><td><b><i18n:message key='citcorpore.comum.datafim' /></b></td></tr>");
		...
		if (lista != null && lista.size() > 0){
			...
			for (TarefaLinhaBaseProjetoDTO tarefaLinhaBaseProjetoDTO : lista) {
				...
				if (tarefaLinhaBaseProjetoDTO.getDataFim() != null
					...
					existeAtrasada = true;
					atvAtrasadas.append("<tr><td><img src=\"" + Constantes.getValue("CONTEXTO_APLICACAO") + "/imagens/b.gif\" border=\"0\"/></td><td>" + nomeProjeto + "</td><td>" + tarefaLinhaBaseProjetoDTO.getNomeTarefa() + "</td><td>" + UtilDatas.dateToSTR(tarefaLinhaBaseProjetoDTO.getDataFim()) + "</td></tr>");
				}
		...
		if (!existeAtrasada){
			atvAtrasadas.append("<tr><td colspan='20'><i18n:message key='carteiraTrabalho.naoExisteTarefaAtrasada' /></td></td>");
		}
