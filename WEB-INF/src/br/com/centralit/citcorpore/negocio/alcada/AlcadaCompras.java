    ...
    public AlcadaDTO determinaAlcada(IDto objetoNegocioDto, CentroResultadoDTO centroCustoDto, TransactionControler tc) throws Exception {
        ...
        for (LimiteAlcadaDTO limiteAlcadaDto : colLimites) {
            ...
            if (requisicaoProdutoDto.getFinalidade().equalsIgnoreCase("I")){
                if (limiteAlcadaDto.getLimiteItemUsoInterno().doubleValue() < valorAprovado.doubleValue())
                    continue;
                if (limiteAlcadaDto.getLimiteMensalUsoInterno().doubleValue() < valorAprovadoMensal.doubleValue())
                    continue;
            }else{
                if (limiteAlcadaDto.getLimiteItemAtendCliente().doubleValue() < valorAprovado.doubleValue())
                    continue;
                if (limiteAlcadaDto.getLimiteMensalAtendCliente().doubleValue() < valorAprovadoMensal.doubleValue())
                    continue;
            }
