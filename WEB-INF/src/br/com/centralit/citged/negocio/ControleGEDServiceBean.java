...
public class ControleGEDServiceBean extends CrudServicePojoImpl implements ControleGEDService {

	private static final long serialVersionUID = -6590764538418976459L;

	protected CrudDAO getDao() throws ServiceException {
		return new ControleGEDDao();
	}
	...
	public Collection listByIdTabelaAndID(Integer idTabela, Integer id) throws Exception {
		ControleGEDDao ctrlGedDao = new ControleGEDDao();
		return ctrlGedDao.listByIdTabelaAndID(idTabela, id);
	}

	public String getProximaPastaArmazenar() throws Exception {
		ControleGEDDao ctrlGedDao = new ControleGEDDao();
		return ctrlGedDao.getProximaPastaArmazenar();
	}
	...
	public Collection listByIdTabelaAndIdBaseConhecimentoPaiEFilho(Integer idTabela, Integer idBasePai, Integer idBaseFilho) throws Exception {
		...
		return gedDao.listByIdTabelaAndIdBaseConhecimentoPaiEFilho(idTabela, idBasePai, idBaseFilho);
	}
	
	public Collection listByIdTabelaAndIdBaseConhecimento(Integer idTabela, Integer idBaseConhecimento) throws Exception {
		ControleGEDDao gedDao = new ControleGEDDao();
		return gedDao.listByIdTabelaAndIdBaseConhecimento(idTabela, idBaseConhecimento);
	}
	
	public Collection listByIdTabelaAndIdLiberacaoAndLigacao(Integer idTabela, Integer idRequisicaoLiberacao) throws Exception {
		ControleGEDDao gedDao = new ControleGEDDao();
		return gedDao.listByIdTabelaAndIdLiberacaoAndLigacao(idTabela, idRequisicaoLiberacao);
	}
            ...
            for (UploadDTO uploadDto : anexos) {
                ...
                if (GED_INTERNO.equalsIgnoreCase("S") && !"S".equalsIgnoreCase(gedInternoBancoDados)) {
                    ...
                    if (controleGEDDTO != null) {
                        ...
                        File arquivo = new File(GED_DIRETORIO + "/1/" + pasta + "/" + controleGEDDTO.getIdControleGED() + "."
                                + Util.getFileExtension(uploadDto.getNameFile()));
    
                        CriptoUtils.encryptFile(uploadDto.getPath(), GED_DIRETORIO + "/1/" + pasta + "/" + controleGEDDTO.getIdControleGED()
    ...
    public ControleGEDDTO getControleGED(AnexoBaseConhecimentoDTO anexoBaseConhecimento) throws Exception{
    	ControleGEDDao ctrlGedDao = new ControleGEDDao();
    	return ctrlGedDao.getControleGED(anexoBaseConhecimento);
    }
}