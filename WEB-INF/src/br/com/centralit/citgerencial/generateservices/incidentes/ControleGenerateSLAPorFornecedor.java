...
public class ControleGenerateSLAPorFornecedor {
	public List execute(Integer idServico, Integer idFornecedor, Integer ano, Integer mes) throws ParseException {
		/*
...
*/
		...
		if(ano != null && mes != null){
			sql = sql + "WHERE (serc.idservicocontrato = ? and cont.idfornecedor = ? and " + AdaptacaoBD.getYear("sol.datahorasolicitacao") + " = ? and " + AdaptacaoBD.getMonth("sol.datahorasolicitacao") + " = ?) ";
		}else{
			sql = sql + "WHERE (serc.idservicocontrato = ?) ";
			sql = sql + " AND (cont.idfornecedor = ?) ";
		}
		...
		if(ano != null && mes != null ){
			parametersValuesBusca.add(idServico);
			parametersValuesBusca.add(idFornecedor);
