...
public class GenerateSLA extends GerencialGenerateService {
	...
	public List execute(HashMap parametersValues, Collection paramtersDefinition) throws ParseException {
		...
		/* Desenvolvedor: Rodrigo Pecci - Data: 31/10/2013 - Horário: 17h40min - ID Citsmart: 120770
		 * Motivo/Comentário:  O servicocontrato no select foi alterado para inner join.
		 */
		...
		try {
			...
			if (listaDados != null) {
				for (int i = 0; i < listaDados.size(); i++) {
					...
					if (dataHoraFim != null) {
						dataHoraFimStr = UtilDatas.formatTimestamp(dataHoraFim);
					}
					Timestamp dataHoraLimite = (Timestamp) row[5];
					String dataHoraLimiteStr = "--";
					if (dataHoraFim != null) {
						dataHoraLimiteStr = UtilDatas.formatTimestamp(dataHoraLimite);
					}
					...
					if (noPrazoInfo.equalsIgnoreCase("*") || noPrazoInfo.equalsIgnoreCase(noPrazoComp)) {
						lstRetorno.add(new Object[] { sla, row[0] + "", row[1], row[2], UtilDatas.formatTimestamp(dataHoraSol), dataHoraLimiteStr, dataHoraFimStr, prioridadeStr, captura, atraso,
			...
			for (Iterator itAux = lstRetorno.iterator(); itAux.hasNext();) {
				...
				if (((String) obj[12]).equalsIgnoreCase(UtilI18N.internacionaliza(request, "citcorpore.comum.sim"))) {
			...
			if (lstRetorno == null || lstRetorno.size() == 0) {
				...
				/**
				 * Foi adicionado comentado o Item acima pois, ao gerar o relatório com a lista vazia
				 * o relatório estava sendo mostrado.
				 * Bruno.Aquino
				 */
			}
			for (Iterator iterator = paramtersDefinition.iterator(); iterator.hasNext();) {
	...
	private void acrescentarNaSqlOLimitadorDeRegistros(StringBuilder sql, Integer maximoRegistros) {
		...
		String valores = " valor1, valor2, valor3, valor4, valor5, valor6, valor7, valor8, valor9, valor10, valor11, valor12, valor13, valor14, valor15, valor16, valor17, valor18, valor19 ";
