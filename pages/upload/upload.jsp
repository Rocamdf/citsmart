...
<script>
	...
	function excluirImagemUpload(path){
		if (confirm(i18n_message("citcorpore.comum.confirme.desejaexcluiranexo"))){
			window.location = '<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/pages/uploadExcluir/uploadExcluir.load?path=' + path;
		}
	}
	...
	function excluirAnexo(id, nomeArquivo){
		if(confirm(i18n_message("citcorpore.comum.confirme.exclusaoanexo"))){
			window.location = '<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/pages/upload/excluirAnexo.do?nameFile=' + nomeArquivo + '&id='+id;	
		}		
	}
	<%
...
<table width="100%" class='tableLess'>
	<thead>
		...
		Collection colUploadsGED = (Collection)request.getSession(true).getAttribute("colUploadsGED");
     	Collection colUploadsGED2 = (Collection)request.getSession(true).getAttribute("colUploadsGED2");
     	Iterator it =null;
        ...
        if(flagGerenciamento!= null && flagGerenciamento.equalsIgnoreCase("S")){      	
			if (colUploadsGED2 == null){
			   colUploadsGED2 = new ArrayList();
			}
			it = colUploadsGED2.iterator();
     	} else{
     		
     	if(colUploadsGED == null){
     	   colUploadsGED = new ArrayList();
     	}
     	it = colUploadsGED.iterator();
     	}
      	%>	
      	<%for(Iterator it2 = it; it2.hasNext();){ %>
      		<%
      		UploadDTO uploadDTO = (UploadDTO)it2.next();
      		%>
		<TR>
			<%
			if(branco){ 
			...
			%>	
			<td >		
					<input type="image" id='btnExcluirAnexo' src='<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/imagens/delete.png' style='cursor:pointer' onclick='excluirImagemUpload("<%=uploadDTO.getPath()%>")' title='<i18n:message key="citcorpore.ged.msg.exluiranexo" /> '/>
			</td>				
			<td >
			...
			<td  title='<i18n:message key="citcorpore.ged.situacao" />'>			
