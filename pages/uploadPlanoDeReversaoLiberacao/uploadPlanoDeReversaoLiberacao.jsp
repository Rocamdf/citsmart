...
<script>
	...
	function excluirImagemUpload(path){
		if (confirm(i18n_message("citcorpore.comum.confirme.desejaexcluiranexo"))){
			window.location = '<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/pages/uploadExcluirPlanoDeReversaoLiberacao/uploadExcluirPlanoDeReversaoLiberacao.load?path=' + path;
		}
	}
...
<table width="100%" class='tableLess'>
	<thead>
		<tr>
          ...
          <th><i18n:message key="citcorpore.comum.versao" /></th>
		</tr> 
	</thead> 
        <%boolean branco = true;%>
      	<%String cor = "#ffffff";%>	
      	<%
		Collection colUploadsGED = (Collection)request.getSession(true).getAttribute("colUploadPlanoDeReversaoLiberacaoGED");
		...
		<TR>
			...
			%>	
			<td >		
					<img src='<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/imagens/delete.png' style='cursor:pointer' onclick='excluirImagemUpload("<%=uploadDTO.getPath()%>")' title='<i18n:message key="citcorpore.ged.msg.exluiranexo" /> '/>
			</td>				
			<td >
			...
			<td  title='<i18n:message key="citcorpore.ged.versao" />'>			
				<%=uploadDTO.getVersao()%>
			</td>
			<%branco=!branco;%>		
        </TR>
