...
<script><!--
	...
	function abreQuestionario(id){
		...
		document.getElementById('frameQuestionario').src ='<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/pages/visualizacaoQuestionario/visualizacaoQuestionario.load?modo=edicao&idQuestionario='+id;
		chamaHistorico();
	}
	...
	function gravar(){
		...
		window.frames["frameQuestionario"].document.formQuestionario.action = '<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/pages/requisicaoQuestionario/requisicaoQuestionario.load?idRequisicao='+idRequisicao+'&idTarefa='+idTarefa+'&idTipoRequisicao='+idTipoRequisicao+'&idTipoAba='+idTipoAba+'&idRequisicaoQuestionario='+idRequisicaoQuest;
	...
	function chamaEdicaoQuestionario(idRequisicao, idQuestionario, idItemParm, idIdentificador, somenteLeitura, subForm, abaSusp, nomeQuest){
		...
		modo = 'edicao';
			...
			document.getElementById('frameQuestionario').src = '<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/pages/visualizacaoQuestionario/visualizacaoQuestionario.load?modo=' + modo + '&idQuestionario=' + idQuestionario + '&idIdentificadorResposta=' + idIdentificador +  '&tabela100=false';
	...
	function criarQuestionario(){
		$('#POPUP_CRIAR_QUEST').dialog('open');
	 	document.getElementById('frameCriarQuestionario').src = '<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/pages/questionario/questionario.load?iframe=true'; 
		...
		<div id="main_container" class="main_container container_16 clearfix">
			...
			<div class="box grid_16 tabs">
				<ul class="tab_header clearfix">
					<li><a href="#tabs-1"><i18n:message key="checklistQuestionario.registrarChecklist" /></a></li>
				</ul>
				<a href="#" class="toggle">&nbsp;</a>
				<div class="toggle_container">
					<div class="block" >
						<div id="parametros">
							<form name='form' action='<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/pages/checklistQuestionario/checklistQuestionario'>
								<div class="columns clearfix">
									<div class="col_25">
										...
										<fieldset >
											<label><i18n:message key="menu.nome.questionario" /></label>
											<div >
									...
									<div>
										...
										<img src="<%=br.com.citframework.util.Constantes.getValue("CONTEXTO_APLICACAO")%>/imagens/add.png" style="padding-left: 8px;">
										<span><i18n:message key="checklistQuestionario.criarQuestionario" /></span>
										</button>
										
										<button type='button' name='btnAtualizar' class="light" 
										onclick='carregaCombo();'
										style="margin: 20px !important;">
										<img src="<%=br.com.citframework.util.Constantes.getValue("CONTEXTO_APLICACAO")%>/template_new/images/icons/small/grey/refresh_3.png" style="padding-left: 8px;">
										<span><i18n:message key="citcorpore.comum.atualizar" /></span>
										</button>
									</div> 
								</div>
								<fieldset>
									<div id="quest" style="height: 600px">
	                            		...
	                            		<h2 class="section">
											<i18n:message key="citcorpore.comum.historico" />
										</h2>
	                            		<div class="col_40">
								...
								<div class="col_100">
									...
									<button type='button' name='btnRelatorio' class="light" 
											onclick='gravar();'
											style="margin: 20px !important;">
											<img src="<%=br.com.citframework.util.Constantes.getValue("CONTEXTO_APLICACAO")%>/template_new/images/icons/small/grey/pencil.png" style="padding-left: 8px;">
							...
							<div id="POPUP_IMPRIMIR_QUEST">
								<form name='formImprimirFormulario' method="POST" action="<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/pages/checklistQuestionario/checklistQuestionario">
