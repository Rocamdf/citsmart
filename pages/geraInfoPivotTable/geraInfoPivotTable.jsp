...
var intervalo;
	...
	<script type="text/javascript" src="<%=CitCorporeConstantes.CAMINHO_SERVIDOR%><%=request.getContextPath()%>/pages/painel/jquery.ui.position.js"></script>
        ...
        <link href="<%=br.com.citframework.util.Constantes.getValue("SERVER_ADDRESS")%><%=br.com.citframework.util.Constantes.getValue("CONTEXTO_APLICACAO")%>/novoLayout/common/theme/css/style-light.min.css" rel="stylesheet" />
</head>
    <style>
        * {font-family: Verdana;}
    </style>
<body>	
    		...
    		function gerarPainel(){
    			...
    			//if(validaData(dataInicio, dataFim)){
    				if (document.formParametros.validate())
    				{
    					...
    					fecharTelaParm();
    	                $.getJSON("<%=br.com.citframework.util.Constantes.getValue("SERVER_ADDRESS")%><%=br.com.citframework.util.Constantes.getValue("CONTEXTO_APLICACAO")%>/pages/geraInfoPivotTable/geraInfoPivotTable.load?parmOK=true&idConsulta=<%=idConsulta%>&" + queryStr, function(mps) {
    	                    $("#output").pivotUI(mps);
    	                });    					
    				}
    			//}
        ...
        <div id='divImpressao'>
        	<button type='button' name='btnImprimir' class='light' onclick='imprimir()'><i18n:message key="cronograma.imprimir" /></button>
        </div><br>
