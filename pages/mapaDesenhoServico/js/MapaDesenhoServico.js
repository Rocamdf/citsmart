...
//			document.form.txtDescricao.value = itemSelecionado.objeto.descricao;			
	...
	var mouseClickedMove = function(e) {
		...
		if(itemSelecionado != null){
			 ...
			 * linha será traçada do ponto da imagem até o ponto do mouse.
			 */
			atualizarCanvas();
	...
	var mouseUp = function(e) {
		...
		//Verifica se a linha foi solta em cima de um outro item para torná-lo filho.
...
//			self.context.closePath();	
			...
			if(filho != null && filho.objeto.idImagemItemConfiguracao != itemSelecionado.objeto.idImagemItemConfiguracao){
				if(itemSelecionado.objeto.idImagemItemConfiguracao == 0){
					...
					filho.objeto.idImagemItemConfiguracaoPai = itemSelecionado.objeto.idImagemItemConfiguracao;
				}				
			}
		}
		if(interval != null){
	...
	var atualizarCanvas = function() {	
		...
		for ( var i = 0; i < self.listaItens.length; i++) {
			...
			if (self.listaItens[i].idImagemItemConfiguracaoPai != null) {
				var imgPai = findImagemById(self.listaItens[i].idImagemItemConfiguracaoPai);
				if (imgPai != null) {
			...
			if(self.listaItens[i] != null){
				if(posicao.x >= self.listaItens[i].posx && posicao.x <= self.listaItens[i].getPosX2() && 
				   posicao.y >= self.listaItens[i].posy && posicao.y <= self.listaItens[i].getPosY2()){					
					//o retorno é um objeto anônimo com a posição do objeto na lista e o objeto em si.
	...
	var connectPoints = function(pai, filho){		
		...
		switch(tipoLinha){
			case TipoLinha.TRES_PONTOS :
				var meioX = centralXPai+(centralXFilho - centralXPai)/2; 
				var meioY = centralYPai+(centralYFilho - centralYPai)/2;				
...
//		this.context.lineTo(this.getXPos(), this.getYPos());
	...
	/**
	 * Apenas para tornar a imagem visíveis após o carregamento. 
	 */
