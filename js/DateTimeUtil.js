...
var DAY_NAMES=new Array('Domingo','Segunda','Ter�a','Quarta','Quinta','Sexta','Sab�do','Dom','Seg','Ter','Qua','Qui','Sex','Sab');
...
DateTimeUtil.isValidTime = function(horaStr){
     ...
     if(hora.length != 5){
         alert('Formato de hora inv�lido (hh:mm)');
         return false;
     }
     ...
     if(h > 23 || h < 0){
         alert('Hora inv�lida(de 00 a 23)');
         return false;
     }
      
     if(m>59 || m<0){
         alert('Minuto inv�lido(de 00 a 59)');
         return false;
     }
