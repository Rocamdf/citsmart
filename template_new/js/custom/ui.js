...
//jQuery UI elements (more info can be found at http://jqueryui.com/demos/)
	...
	// Navigation accordions
		...
		$("ul.menuoverlay").parent("li").addClass("has_menuoverlay");
		
		$(".has_menuoverlay > a").click(function(){
			var menuType = ($(this).parent().hasClass("open"));
			if (menuType != true){
				$(this).parent().siblings().removeClass("open").children("ul.menuoverlay").slideUp();
			}
			$(this).parent().toggleClass("open").children("ul").animate({width: "toggle"});
			exit;
		});
